...
- **Product pages no longer show validations that do not apply to the product.** [view:/migrate] [persona:ops] [persona:grc] A product-to-certificate link now stays only if the certificate names the product (not just the vendor, a shared generic word such as "Series", a build variant, or a different release) or the product's own documentation cites the certificate number. 99 links were retired: 38 pointed at FIPS 140-2 certificates, 41 were not backed by the record (links to another product line, another vendor's module or another release), 1 pointed at a CAVP validation NIST no longer lists, and 19 gave way to a newer certificate for the same module. 38 links were added, mostly to newly listed validations, and a module that approves a post-quantum algorithm is now preferred over a newer one that does not.

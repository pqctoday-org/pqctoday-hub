...
- **Product pages no longer show validations that do not apply to the product.** [view:/migrate] [persona:ops] [persona:grc] A product-to-certificate link now stays only if the certificate names the product or the product's own documentation cites the certificate number. 51 links were retired, most of them to FIPS 140-2 certificates. The rest were links between different builds of the same library, plus a few hand-pinned links that no official record supports.

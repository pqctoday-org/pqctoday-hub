...
- **Protocol Support spec links open the in-app Library entry** [view:/algorithms] [persona:architect] [persona:developer]: clicking an RFC or draft in a protocol's Specifications (or a matrix cell) now opens that document's Library entry inside the app — which itself carries the external link — instead of bouncing you straight to the raw external RFC page.

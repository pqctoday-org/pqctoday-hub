...
- **`SourcePassagesDrawer` + `useSourcePassages` hook** [view:/library][view:/migrate][view:/threats]: Collapsible per-record drawer that surfaces W3C PROV-DM source passages from the RAG corpus (`prov.source_passages[]`) inline on data cards. The hook resolves chunks via the existing `UnifiedSearchService` singleton (no second corpus fetch) and settles synchronously when the corpus is already loaded. Wired to Library, Migrate, and Threats cards with the appropriate domain-prefixed chunk ids (`library-{referenceId}`, `software-{productId|slug}`, `threat-{threatId}`). Timeline deferred — chunk ids are opaque sequential indices and would need a generator-side stable-key refactor.

...
- **A full crypto-agile KMIP control plane, running entirely in your browser** [view:/playground/cacp] [persona:developer] [persona:architect]: a new Crypto Lab workshop drives a real KMIP 3.0 control plane and SoftHSM key engine compiled to WebAssembly — no server, no Docker. Walk three guided planes: pick a policy and watch the same operation get allowed, denied, or transparently auto-rekeyed; run Create → Activate → Sign/Verify (or Encapsulate/Decapsulate) as genuine KMIP requests and inspect the TTLV wire response in a tree view; then see the PKCS#11 keystore the engine actually populated alongside a cross-plane audit trail for every operation.

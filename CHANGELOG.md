...
- **New library document — Circle's Post-Quantum Security Roadmap (2026).** `Circle-PQC-Roadmap-2026` — co-authored by Circle (Belenkiy, Le, Liao, Sehrawat, Rotaru, Gorbunov, Sheth, Logelin, De Abreu) and Dan Boneh (Stanford). Catalogs seven quantum attack vectors on the full blockchain stack: at-rest forgery via EVM `ecrecover` (every account that has ever signed is exposed), retroactive privacy loss (HNDL), consensus disruption, chain history rewrite, P2P session compromise (libp2p/Noise), RPC interception, and on-spend key recovery. Readiness phase: SLH-DSA-SHA2-128s precompile on Arc mainnet + X-Wing hybrid KEM (ML-KEM-768 + X25519) for TLS 1.3 and encrypted memos + TEE private execution layer (AWS Nitro/SGX/TDX). Transition phase: dual-mode USDC smart contracts, post-quantum `ecrecover` override, multi-sig cold storage, encrypted mempool. Switch phase: full PQ validator signatures, account recovery (dual-key BIP-39, TEE-attested ZK, off-chain). Policy section covers quantum flag day regulatory implications for stranded USDC assets and `escheat` doctrine. Wired to modules: digital-assets, quantum-threats, slh-dsa, hybrid-crypto, crypto-mgmt-modernization, pqc-risk-management, migration-program, crypto-agility, kms-pqc, confidential-computing, stateful-signatures.
- **Visual consistency audit — all 55 learn modules now have `pqcstd_` format infographics.** Four modules were missing visuals at every fallback tier (slh-dsa, pki-enrollment-protocols, mls-group-messaging, pqc-candidates); four new `pqcstd_` PNGs added. MLSGroupMessaging was also missing its Visual tab entirely — tab and `ModuleVisualTab` wiring added. All modules now render the infographic correctly for all personas (including curious mode via `CuriousModuleView` / `CuriousStackCarousel`) on both mobile and desktop.
...
- **Community leader detail — external URL support.** Leader profile popover and side panel now render `keyResourceUrl` entries that start with `http` as external links (with `ExternalLink` icon, opens in new tab) rather than routing to the internal library search. Reference IDs continue to link to `/library?ref=…` as before. Required for profiles (like Steven Vaile's) whose key resource is a direct PDF URL.
- **Five new Red Hat products in Migrate catalog.** `pqc_product_catalog_05302026.csv` adds: **RHEL 10.2** (ML-KEM + ML-DSA + SLH-DSA, GA May 6 2026), **RHEL 9.8** (ML-KEM + ML-DSA backport, GA May 6 2026), **Red Hat Certificate System 11.0** (ML-DSA PKI signatures, GA May 6 2026), **Red Hat Trusted Artifact Signer** (ML-DSA via Sigstore, PoC complete, GA target early 2027), and **Red Hat Ansible Automation Platform** (PQC inherited from RHEL, rolling out 2026–2027). All sourced from Red Hat May 2026 press release and "Building the Levee" blog post.
...
- **URL persistence and deep-links.** The selected Work Role is written to `?role=<id>` so shared links land on the right role. Navigating to `/learn?view=nice` opens the NICE view directly; switching away clears `?view=` so refresh returns to your normal view. The Assess Report footer now includes "Explore your curriculum in NICE Framework view →" linking to `/learn?view=nice&role=<topRole>`.
- **Mobile role selector.** On narrow viewports the chip strip is replaced by a `<FilterDropdown>` so the role picker doesn't wrap across three lines.
- **IAM Specialist coverage expanded.** Role now covers 10 modules (was 5): added code-signing, email-signing, kms-pqc, hsm-pqc, and merkle-tree-certs. Also added the missing `mls-group-messaging` module to the NICE map (CA-NETDEF + CA-CRYPTO) — all 55 modules now appear in the view.
- **CA color differentiation.** Network Defense (CA-NETDEF) uses blue (info) instead of green so it is visually distinct from Secure Programming (CA-SECPROG).
...
- **"Try in Playground" buttons on algorithm cards for all four Round 2 families.** MAYO, CROSS, OV/UOV, and SNOVA algorithm cards on the `/algorithms` page now carry a direct **Try** link that opens the Playground pre-loaded on a representative variant (MAYO-1, CROSS-RSDP-128-balanced, OV-Ip, SNOVA-24-5-4) in the sign/verify tab. FN-DSA, FrodoKEM, and HQC cards also receive "Try" links they were previously missing.
...
- **Three new library documents.** "Preparing for Quantum Technologies" (G7 Central Banks / Bank of Canada + Banque de France, May 2026), "Quantum Threat: The Trillion-Dollar Security Race Is On" (Citi GPS, January 2026), and "Securing Financial Transactions in the Quantum Era: A Critical Review of Hybrid Cryptographic Innovations" (Sandhu & Sharma, _Franklin Open_ / Elsevier, May 2026, open access CC BY-NC-ND) — all enriched and searchable in `/library`.
...
- **Library persona-overwhelm remediation (P04 audit, persona-overwhelm-p0)** [persona:executive][persona:developer][persona:architect][persona:ops][persona:researcher][persona:curious][view:/library]: Four-PR remediation against the 2026-05-22 library page audit. (PR 1 — P0 multi-category persona filter) `PERSONA_LIBRARY_CATEGORIES` upgraded from a half-wired hint (single seed + tie-break sort) into a real persona-aware default filter; sidebar selection now defaults to "All" so the persona-preferred set narrows the corpus when no explicit category is picked. New `?prefs=off` escape hatch toggles back to the full corpus and the result-count area renders a "Showing N matched to your role · See all 833" reset banner when the persona narrowing is active. `ops` preferred categories extended to include `NIST Standards` + `Algorithm Specifications` so FIPS 203/204/205 land inside the default ops set. `CategorySidebar` accepts `personaPreferredActive` and visually demotes the `All` pill when narrowing is active. Persona-switch effect strips a lingering `?prefs=off` so new defaults always apply. Researcher unchanged — empty preferred array short-circuits to "no narrowing", confirmed AC-5. (PR 2 — picks panel + persona-aware sort) New `PersonaPicksPanel` (extracted from the curious-only inline JSX, parameterised by persona/picks/title/subtitle) renders curated entry lists for `curious` ("Start here", 3 picks from `libraryCuriousPicks` — trimmed from 5 → 3 under P2.b, dropping FIPS-204/205 which are reachable via the FIPS-203 ↔ IR-8547 hub-and-spoke), `executive` ("Boardroom set", 7 picks in new `libraryExecutivePicks.ts` — NIST CSWP 39, NIST IR 8547, NSA CNSA 2.0, ANSSI PQC Follow-up, BSI TR-02102-1, EU-REC-2024-1101, UK-NCSC Migration Timelines), and `ops` ("Cert-relevant set", 6 picks in new `libraryOpsPicks.ts` — FIPS 203/204/205, NIST SP 800-208/-227, CMVP Management Manual). Visible cap 5 cards + `Show all N` expander. New persona-aware default sort: executive=newest · developer=referenceId · architect=urgency · researcher=mostCited · ops=urgency · curious=newest; explicit `?sort=` still wins; `?sort=` only persisted when it differs from the persona default. New `mostCited` sort + `citationCount` derivation in `libraryData.ts` — single O(N) pass over `dependencies` reverse edges; ties fall back to `lastUpdateDate` so empty corpora still sort stably. (PR 3 — drawer consolidation + chips) Region/Industry single-select drawer dropdowns removed; their predicate branches dropped; one-release migration shim on mount converts legacy `?region=X` → `geo[]` and `?ind=X` → `sector[]` (multi-select equivalents) with a dev-only `console.warn`, then deletes the old params so URLs converge. Drawer now exposes five axes — Organization, Doc Status, Country/Region (the `GeoFilter` taking over from the single-select), Sector, Trust tier — and for `executive` + `curious` collapses Sector + Trust tier behind an `Advanced (Sector, Trust tier)` expander; researcher + architect see all five expanded by default. New primary-row `Cert-relevant ({N})` chip (next to `CSWP 39 (189)`) filters to the curated `LIBRARY_OPS_PICKS` allowlist; new Lifecycle pill row (`Final` / `Draft` / `Expired` / `Withdrawn` as a `role=radio` group) binds to the existing `lifecycleBucket` state so the drawer Doc Status dropdown and the pills stay in sync. URL-persisted: `?cert=1`. (PR 4 — Curious minimum-viable mode) For `persona === 'curious'`, the page shell — ActivityFeed, ContentUpdatesFeed, CategorySidebar, Controls Bar, Active Chips, Results count, content grid — is hidden behind a single `Browse the full library` CTA below the picks panel so only the header + 5 picks + button render above the fold. Shareable via `?expand=1`. Other personas always see the full shell. (Validation) `tsc --noEmit` clean; `eslint` 0 errors; `audit:matrix-refs` PASS; `npm run build` clean; `LibraryView.test.tsx` 22/22; full suite 3077/3079 (2 pre-existing failures in `corpus-trust-invariants.test.ts` for `library-G7-CB-QT-Financial-2026` + `library-Citi-GPS-Quantum-Threat-2026` embeddings-meta drift — unrelated to any file touched by this work). Browser-verified at `localhost:5300/library` across executive (matched banner, Boardroom set + 7 picks, Cert-relevant chip, Lifecycle pills), researcher (no banner, no panel — AC-5 preserved), curious (Start here + Browse the full library expander; click → `?expand=1` + shell reveals). (Follow-up gap pass) Closed four adjacent gaps after the main 4-PR landing: (1) E2E spec hardened — `test.describe.configure({ mode: 'serial' })` added to `e2e/library.spec.ts` so it passes under default parallel local Playwright runs (CI already pins `workers: 1`); 8/8 scenarios pass against both the canonical `playwright.config.ts` (port 5175, auto-started dev server) and the local override. (2) Dead-code cleanup — removed unused `activeRegion` / `activeIndustry` state, `storeIndustry` destructure, their URL-sync references, and the `ind`/`region` overrides from `syncFiltersToUrl`. The migration shim itself stays (still converts legacy `?region=`/`?ind=` URLs into `geo[]`/`sector[]`). (3) Accessibility polish — `role="status" aria-live="polite"` on the matched-banner wrapper so screen readers announce persona-narrowing changes; WAI-ARIA roving tabindex + ArrowRight/ArrowLeft activation on the Lifecycle radiogroup (with `tabIndex={-1}` on the group to satisfy `jsx-a11y/interactive-supports-focus` without breaking the radio pattern); `aria-controls="library-advanced-drawer"` on the Advanced expander + `role="region"` + `aria-label` on the wrapped advanced block. Four new a11y unit tests cover all three. (4) Picks-data CSV gap formally flagged — `libraryExecutivePicks.ts` and `libraryOpsPicks.ts` headers now name the deferred CSV-side data work precisely: `CISA-Quantum-Readiness-Roadmap` (executive set, would bring it to 8 picks), four most-cited CC PP-Modules + `CMVP-IG-D.K` (ops set, would bring it to ~11 picks). Adding those rows to `library_MMDDYYYY.csv` is a follow-up data task; once landed, the picks files extend by line addition only. Total Library tests after this pass: **64 unit (49 + 6 + 9) + 8 E2E.** (Picks-data closeout) New library CSV row `CISA-Quantum-Readiness-Roadmap` added in `library_05222026_r2.csv` — the August-2023 CISA/NSA/NIST joint quantum-readiness migration roadmap — wired into `LIBRARY_EXECUTIVE_PICKS` to hit the audit plan's full 8-entry target. Audit plan's `CMVP-IG-D.K` reference resolved to `NIST-FIPS140-3-IG-PQC` (already in the CSV — the FIPS 140-3 Implementation Guidance for PQC) and added to `LIBRARY_OPS_PICKS` (now 7 entries). The Cert-relevant chip count auto-updates from 6 → 7 because the chip filters by `LIBRARY_OPS_PICKS` allowlist intersection. Only remaining picks-data gap: 4× CC Protection Profile Modules in ops picks (audit plan named them by category, not by specific ID; deferred pending product input on which four are most cert-relevant).
- **Compliance persona-overwhelm audit response — P2 follow-ups (sub-facet hint payload + aria-current + click/dismiss analytics)** [view:/compliance][persona:executive][persona:developer][persona:architect][persona:ops][persona:researcher][persona:curious]: Three "nice-to-have" items from the same 2026-05-22 audit plan, all additive on top of the P0 + P1 baseline. (a) **P2-1 — sub-facet hint payload**: `ComplianceHint` schema gains optional `subFacet?: { rtab?: 'fips' | 'acvp' | 'cc'; pqcOnly?: boolean }`. Every industry/region hint in `INDUSTRY_COMPLIANCE_HINT` + `REGION_COMPLIANCE_HINT` now declares the cert-type sub-tab that matches its `sectionLabel` (Finance/Healthcare/Gov/Aerospace/Energy/Retail → `fips`; Telecom/Technology → `acvp`; Automotive + EU region → `cc`). `PersonaHintCta.onNavigate` signature extends to `(section, subFacet?)`; `handlePersonaHintNavigate` forwards `subFacet.rtab` into `syncFiltersToUrl`. Cross-tab pre-selection required a small extension to `useComplianceUrlState.syncFiltersToUrl`: the landscape branch now honors an explicit `rtab` override (it was previously written only on Records destinations), so a Finance executive landing on Certification Schemes already has `?rtab=fips` set for the moment they pivot to Records. Live verification: Finance CTA → `?tab=certification&rtab=fips`; Automotive CTA → `?tab=certification&rtab=cc`. (b) **P2-2 — `aria-current` on about-strip summary when expanded**: `AboutThisPageStrip`'s `<summary>` gets `aria-current="true"` while the strip is open, absent (`"false"` by default) when collapsed. Screen readers now read the open strip as the active landmark rather than as decorative chrome. (c) **P2-3 — persona-hint click vs dismiss analytics**: existing `logComplianceFilter('PersonaHint', section)` retained for backward compatibility with dashboards. New `PersonaHintCtaClick` event carries the industry/region→section identity (e.g. `industry:Finance & Banking→certification`, `region:eu→certification`) so the editorial mapping can be tuned — if Telecom hints to ACVP but Telecom users never click, that's a signal to re-examine `INDUSTRY_COMPLIANCE_HINT['Telecommunications']`. New `PersonaHintDismiss` event fires on the X-button with the same identity label so click-through can be sliced against dismiss-through per industry. (d) **Tests** — 3 new unit tests (sub-facet click → industry→section value, dismiss → identity value, `aria-current` on open strip); 60 Compliance tests total, all passing. tsc + lint clean (only the pre-existing `security/detect-object-injection` warning at `ComplianceView.tsx:1136` remains, unchanged from the P0 baseline). Live preview verified on the running dev server. With P2 landed, every item from the persona-overwhelm audit plan (P0-1 → P0-5, P1-1 → P1-4, P2-1 → P2-3) is now shipped.

- **Compliance persona-overwhelm audit response — chrome collapse + persona CTA + executive CSV + persona-gated deadline timeline** [view:/compliance][persona:executive][persona:ops][persona:curious][persona:developer][persona:architect][persona:researcher]: Implementation of the 2026-05-22 persona-overwhelm audit plan (`pqctoday-priv/docs/platform/ux/page-audits/2026-05-22-persona-overwhelm/plans/compliance.md`). Cuts the steady-state row count above the tab bar from eleven to five for returning users; converts the only persona-aware editorial copy on the page (the industry/region hint banner) into a one-click navigation primitive; surfaces the audit-ready CSV export inside the executive For-You body instead of as an icon in the header cluster. All P0 + P1 items shipped; no data shape or store schema changes. (a) **P0-1 — `PersonaHintCta`** (`src/components/Compliance/PersonaHintCta.tsx`): replaces the static "Finance & Banking focus: Certification Schemes → FIPS 140-3" editorial copy with a gradient `<Button variant="gradient">` whose `onClick` calls `handlePersonaHintNavigate(complianceHint.section)`. Mirrors `handleCswp39Jump`'s rAF scroll pattern so the tab-bar anchor lands smoothly. Logs analytics via `logComplianceFilter('PersonaHint', section)`. Carries `data-workshop-target="compliance-persona-hint-cta"` so the in-app Workshop step list can deep-link to it. (b) **P0-2 — `AboutThisPageStrip`** (`src/components/Compliance/AboutThisPageStrip.tsx`): wraps `LearningFrameBanner`, `GlossaryStrip`, and `ContentUpdatesFeed` in a single `<details>` titled "About this page". Counts as one row regardless of open/closed state. Persists open/closed in `compliance-about-expanded-v1`. Defaults to expanded on a true first visit (heuristic: neither `compliance-intro-dismissed-v1` nor `compliance-about-expanded-v1` set), collapsed thereafter. (c) **P0-3 — intro/hint dedup**: suppresses the three-column "New to PQC compliance?" intro card whenever `complianceHint && complianceHintLabel` is truthy, even when `introDismissed === false`. Eliminates the duplicate onboarding chrome a first-time Finance executive saw (abstract three-pillar intro AND the strictly-better targeted hint). Intro card remains for the no-profile / no-persona case. (d) **P0-4 — executive in-body CSV export**: adds a labelled `<Button variant="outline" size="sm">` reading "Export audit-ready summary (CSV)" inside `ExecutiveTimelineView`, beneath `RegulatoryClock`, with a one-line "Generates a board-ready CSV of every mandate that applies to your profile" explainer. Reuses the existing `handleExportCsv` callback (lifted into a prop drilled through `ForYouSection`). Header export remains functional for non-executive personas. Filename and column set unchanged. (e) **P0-5 — `DeadlineTimelineGate`** (`src/components/Compliance/DeadlineTimelineGate.tsx`): persona-aware wrapper around `DeadlineTimeline`. Executive / ops / null → full timeline (current behaviour). Curious → one-line narrative summary: "PQC mandates land between {earliestYear} and {latestYear}; the next hard deadline is {framework.label} ({year})." Developer / architect / researcher → closed `<details>` disclosure that mounts the existing component on click (zero DOM cost when collapsed). Data stays reachable for technical personas via the Landscape tab's deadline-phase filter and the CSWP.39 cross-walk. (f) **P1-1 — per-industry/region persona-hint dismissal**: adds an X-dismiss button to `PersonaHintCta` (its `onDismiss` prop was already plumbed). localStorage key `compliance-persona-hint-dismissed-industry:{label}` (or `…-region:eu`) so changing industry/region re-prompts. Implementation: `useMemo`-derived flag keyed on `personaHintDismissKey` + a `personaHintDismissTick` that bumps after a write — satisfies `react-hooks/set-state-in-effect` without falling back to a side-effecting `useEffect`. (g) **P1-3 — mobile trust-tier filter relocation**: `AboutThisPageStrip` gains an optional `headerSlot` prop rendered in the `<summary>` row with click `stopPropagation` so the filter doesn't toggle the strip. `ComplianceView` feeds it a `<TrustTierFilter>` wrapped in `<span className="md:hidden">`. Desktop tab-bar instance unchanged (its container is already `hidden md:block`). (h) **P1-4 — `aria-live` announcement on CTA navigation**: visually-hidden `<span role="status" aria-live="polite" aria-atomic="true">` inside `PersonaHintCta`. `onClick` populates it with `Navigated to {sectionLabel}` and clears after 1500 ms so the region doesn't re-announce the same string on unrelated re-renders. Screen-reader users now hear the tab change instead of a silent URL/DOM swap. (i) **P1-2** (`data-workshop-target="compliance-persona-hint-cta"`) — done as part of (a); listed in the plan as P1 but landed with P0-1. (j) **Tests** — 9 new unit tests covering persona-hint CTA navigation + analytics, intro suppression with hint, intro presence without hint, executive in-body CSV visibility under persona/profile seeding, curious narrative line, developer disclosure (closed by default), per-industry dismissal + cross-industry re-prompt, aria-live announcement on click; 57 Compliance tests total, all passing. (k) **Acceptance against audit** — verified live in browser preview at desktop (1280px) and mobile (375px): row count above tab bar drops to ≤5 for the returning-user case (PageHeader · AboutThisPageStrip · PersonaHintCta · DeadlineTimelineGate · Tab bar); persona CTA click sets URL `?tab=certification` and fires `PersonaHint:certification` analytics; intro card absent when hint resolves, present otherwise; executive on For-You tab sees the labelled CSV button without scrolling past `RegulatoryClock`; curious gets the narrative line; developer gets the closed disclosure; mobile trust-tier in about-strip summary visible at 375px, desktop tab-bar instance hidden; dismiss flow persists per-industry with cross-industry re-prompt confirmed end-to-end. No data shape, store schema, or route contract changes; additive to navigation and conservative on chrome.

- **NIST Round 2 Additional Signatures + FrodoKEM-SHAKE + FN-DSA-padded in Playground** [view:/playground]: `@oqs/liboqs-js@0.15.1` ships four additional-signature families and several variant gaps that were previously unwired — all are now interactive in the key generator, sign/verify tab, and deep-link URL scheme. **MAYO** (MAYO-1/2/3/5): multivariate quadratic signatures with the smallest public keys of any MQ scheme (MAYO-2: 5.4 kB pub, 180 B sig at Level 1); notable for constrained-device and IoT deployments. **CROSS** (CROSS-RSDP-128/192/256-balanced + CROSS-RSDPg-128/192/256-balanced): code-based signatures structurally distinct from lattice or hash-tree constructions; 6 representative balanced variants across both syndrome-decoding families. **OV / UOV** (OV-Ip, OV-Is, OV-III, OV-V): multivariate Oil-and-Vinegar base variants at all three NIST security levels. **SNOVA** (SNOVA-24-5-4, SNOVA-29-6-5, SNOVA-49-11-3): structured-MQ representatives at Levels 1/3/5. Additionally wired: **FN-DSA-padded-512 / FN-DSA-padded-1024** — Falcon constant-time (padded) signing variants that eliminate the timing side-channel present in standard Falcon; labeled "(CT)" in the dropdown to make the security property discoverable. **FrodoKEM-640/976/1344-SHAKE** — SHAKE-based FrodoKEM variants complementing the existing AES variants already in the wrapper. Implementation: `liboqs_sig.ts` gains 19 new factory imports + switch cases + exported type constants (`MAYO_ALGORITHMS`, `CROSS_ALGORITHMS`, `OV_ALGORITHMS`, `SNOVA_ALGORITHMS`, expanded `FALCON_ALGORITHMS`); `liboqs_kem.ts` gains 3 FrodoKEM-SHAKE cases; key generation routing (`useKeyGeneration.ts`) extended to dispatch MAYO/CROSS/OV/SNOVA to `LIBOQS_SIG`; sign/verify routing (`useDsaOperations.ts` `isLiboqsSigAlgorithm`) extended likewise; `SignVerifyTab.tsx` `isSign` and scheme-info display updated; `KeyGenerationSection.tsx` dropdown gains 22 new items with Round 2 labels; `SettingsProvider.tsx` deep-link normalization handles `?algo=MAYO-2`, `?algo=CROSS-RSDP-128-balanced`, etc. correctly. No new npm dependencies — all algorithms were already compiled into the installed `@oqs/liboqs-js@0.15.1` bundle. Unit tests: 87 sig tests pass (mocks + constants + full keygen→sign→verify cycle for all 17 new algorithm names).

- **Algorithm page CTAs for Round 2 Additional Signatures + missing families** [view:/algorithms]: `algorithmCtaMap.ts` prefix table extended with seven new entries so `AlgoCtaStrip` renders a "Try" (FlaskConical) button on cards whose `pqcName` prefix previously fell through to the fallback. New entries: `MAYO` → `/playground/interactive?algo=MAYO-1&tab=sign_verify`; `CROSS` → `?algo=CROSS-RSDP-128-balanced&tab=sign_verify`; `UOV` → `?algo=OV-Ip&tab=sign_verify`; `OV-` → same; `SNOVA` → `?algo=SNOVA-24-5-4&tab=sign_verify`. Existing entries backfilled: `HQC` gains `try: ?algo=HQC-128&tab=keystore`; `FrodoKEM` gains `try: ?algo=FrodoKEM-640&tab=keystore`; `FN-DSA` gains `try: ?algo=FN-DSA-512&tab=sign_verify`. All deep links use the `?algo=&tab=` convention already normalised by `SettingsProvider`.

- **Business + Curious UX — Phase 0 unblock + instrumentation** [persona:executive][persona:developer][persona:architect][persona:ops][persona:researcher][persona:curious]: Mechanical pass that unlocks the persona-tuned surface without changing any visual design. (a) Curious is now nav-allowed on `/algorithms`, `/playground`, `/migrate`, `/patents`, `/openssl` (previously hard-gated); Executive gains `/algorithms`; Developer KPI weights extended to `change-failure-rate` + `canary-coverage` so the Business Center scorecard balances correctly for developers. (b) New `<PreviewBanner>` (`src/components/common/PreviewBanner.tsx`) with `pageContext` prop + sessionStorage-backed dismiss + `Preview Banner Shown / Dismissed` telemetry, mounted on Business Center, Compliance, Migrate, OpenSSL Studio, Patents, and the BC route guard. (c) Every persona-relevant analytics helper (`personaLabel`, `logBookmarkToggle`, `logAchievementUnlocked`, `logQuizAnswer`, `logComplianceFilter`, `logMigrateAction`, `logAssessStart/Step/Complete/Reset`, `logReportViewed/Cta/ShareLinkOpened`, `logAlgorithmView`, `logPersonaSwitchClicked`) now carries the active persona dimension via `personaLabel()` so the entire engagement funnel can be sliced per persona in GA4. (d) New `useUrlPersonaOverride` hook + `UrlPersonaOverride` mount in `App.tsx` accepts `?persona=<id>` query overrides, validates against the `PersonaId` union, applies via `setPersona`, fires `Persona Selected:switch`, and strips the param (preserves other params) — enables shareable role-flavored URLs like `/business?persona=developer`. (e) `logQuizAnswer` wired into the three quiz components (`useQuizState.submitAnswer`, `StandardsBodies/ScenarioChallenge`, `DataAssetSensitivity/ClassificationChallenge`). (f) Per-page persona events on Algorithms (card opens + filters), Learn Dashboard (module-card clicks), Playground (per-tool open with tool id), Business Center (zone select / open / collapse + artifact ZIP export with count), Compliance (tab/filter changes via wrapped helper), Migrate (filter + table actions via wrapped helper), Assess wizard (start / step / complete / reset), Report (CTA clicks + viewed), Library (item open + persona-attributed search), Leaders (card open / close with leader id), Landing hero (primary + secondary CTA with destination path). (g) Assess wizard URL ↔ state mirror — `currentStep` written back to `?step=` on every change (`replace:true`) so refresh / share preserves wizard position. (h) Report shareable URLs — verified DONE; the existing `?share=<base64>` token path + legacy single-letter param fallback already covered. (i) New `src/data/exampleReport.ts` exposes a base64url-encoded sample share token (`Finance · United States · RSA + ECDSA · planning`) and Report empty state surfaces "See an example report" link for curious. (j) PersonaChip click-handler now fires `Persona Switch Clicked:nav` ahead of `Persona Selected:switch`, completing the picker → mis-route → switch funnel against the `<8%` switch-rate KPI in plan §0.8.
- **Business + Curious UX — Phase 1 round 4 (heavy refactors + closeout)** [persona:executive][persona:curious][persona:developer][persona:architect][persona:ops][persona:researcher][view:/][view:/library][view:/playground][view:/compliance][view:/business][view:/assess]: Five shipped per-page integrations plus a final audit pass. Closes every heavy-layout item in the Phase 1 plan. (a) **Curious starter content data files** [view:/library][view:/playground] — new `src/data/libraryCuriousPicks.ts` carries 5 canonical PQC docs hand-picked as first-read material (NIST IR 8547, FIPS 203/204/205, RFC 9964), rendered in a "Start here" callout above the LibraryView activity feed when persona is curious. New `src/data/playgroundCuriousDescriptions.ts` overrides workshopRegistry tool descriptions with plain-English copy for 6 starter-friendly tools (qrng-demo, rng-demo, tls-simulator, hybrid-encrypt, binary-signing, envelope-encrypt) when persona is curious. (b) **Maturity tier promoted to primary scorecard label** [view:/] — for personas with a registered tier override (executive, curious per CC-13), the active ScoreCard header now renders the persona-flavored tier as the primary semantic label (e.g. "Aware" or "Briefed") with the generic belt name + color swatch as secondary annotation. Developer / architect / ops / researcher continue to see the existing `{belt.name}` rendering. The change is one ternary in the same h2-level node, so all existing scroll-targets and the `id="scorecard-heading"` anchor are preserved. (c) **Compliance regulator focus view (left-rail master-detail)** [view:/compliance] — biggest layout refactor in the Phase 1 plan. New `src/components/Compliance/FrameworkFocusView.tsx` renders applicable frameworks as a left-rail list with a right-pane detail panel: PQC shield-icon, deadline, optional "★" emphasis marker, active-row indicator on the left; framework label + deadline + countries + description + notes + industries chips + curious preface on the right. Mounted via a "Focus view" toggle button on `LandscapeTab` — only enabled on the regulations facet where master-detail makes sense. Toggle fires `Compliance · Focus Mode On / Off`. No changes to ComplianceLandscape internals — the focus view is an additive sibling. 6 unit tests cover empty-state, default selection, initialFrameworkId honored, rail-click switches pane + fires callback, Back-to-grid conditional rendering, PQC-required sort priority. (d) **BC zone focus mode** [view:/business] — adds a "Focus / Show all" toggle to the left rail header of the Business Center zone area. When focusMode is on, the right pane filters `CSWP39_ZONE_ORDER` to just the active zone — closed zones disappear entirely so the active one gets full vertical space; left rail stays visible for one-click zone switching. Toggle fires `Business Center · Zone Focus On / Off` telemetry. Default unchanged. (e) **Curious 5-question wizard variant** [view:/assess] — new `CURIOUS_STEP_KEYS` set (industry, country, sensitivity, compliance, migration) branches `AssessWizard`'s step selection when `selectedPersona === 'curious'`. Drops crypto + infra (technical-detail) + timeline (deadline mental model) from the 8-step quick path, keeping the 5 highest-signal questions that still produce a useful report. Deep-link `?step=` clamp, `steps` useMemo, and `stepTitles` (derived from STEP_TITLES_FULL via ALL_STEPS position lookup) all branch the same way. The variant trigger is `selectedPersona === 'curious'` only — a developer with `experienceLevel = 'curious'` still gets the comprehensive flow with proficiency-based "I don't know" hints, preserving the existing personaConfig orthogonality. All 103 Assess tests pass. (f) **Audit closeouts** — verified done via existing infrastructure (no new code needed): P06-P1-01 (next-module CTA already honors `persona.recommendedPath` at `NextModuleCTA.tsx:33-46`), P06-P1-02 (`logModuleStart` / `logStepComplete` / `logModuleComplete` already wrap with `personaLabel`), P06-P1-03 (`CuriousSummaryBanner.tsx` + `CuriousModuleView.tsx` already exist). Deferred as a unified renderer-pass task: P15-P1-07 (Report glossary auto-link) + P06-P1-04 (module glossary auto-link) + P18-P1-01/02 (About outbound link wrapping). Total Phase 1 unit-test count this branch: **63 new tests; every affected suite stays green.** Phase 1 is meaningfully complete — every shared component, every persona analytics path, every density-relief surface, every Curious unlock, the Board Pack export, persona suggestion, hidden-by-role footers, compliance tabs/emphasis/CI-gate/validation-gantt/cert-panel/focus-view, "Why we ask" rationale, maturity tier promotion, BC transparency footer + zone focus mode, and curious starter content + 5-Q variant — all shipped. Remaining 3 net-new tasks all require the same renderer-pass infrastructure and benefit from a future unified pass.

- **Business + Curious UX — Phase 1 round 3 (Report + Assess + BC closeout)** [persona:executive][persona:curious][persona:developer][persona:architect][persona:ops][view:/report][view:/assess][view:/business]: Four shipped per-page integrations plus an audit pass that closed out the remaining backlog. (a) **Board Pack ZIP export** [view:/report] — new `src/services/boardPackBuilder.ts` assembles a downloadable ZIP containing a `board-pack/` folder with six files: `README.md`, `executive-summary.md` (risk score + level + narrative + top-3 actions), `key-findings.md`, `recommended-actions.md` (grouped Immediate / Short-term / Long-term, sorted by priority), `compliance-deadlines.csv` (RFC-4180 quote escaping), `profile.json` (machine-readable replay of the assessment inputs). New `BoardPackExport` button mounted next to the existing "Print Executive Brief" in the Report export-actions row — three-state UX (idle → busy spinner → done checkmark) + `Report · Board Pack Exported` telemetry carrying industry + risk-level + persona dimensions. Pure data builder, no new heavy deps (uses the already-installed `jszip`). 7 new unit tests cover folder contents, exec summary, action ordering, CSV escaping, null-PQC handling, profile.json shape, and empty key-findings fallback. (b) **End-of-wizard persona suggestion card** [view:/report] — new `PersonaSuggestionCard` runs `inferPersonaFromAssessment` against the user's completed assessment and surfaces a non-blocking banner when the inferred persona differs from `selectedPersona`. "Switch to X" updates the persona store + fires `Persona Selected:assessment` and `Persona Suggestion · Switched`; "Stay with Y" writes a 30-day suppression key to `localStorage`. Suppression check runs once at mount via `useState` lazy initializer (keeps render pure per `react-hooks/purity`). Mounted in `ReportView` between `MaturityTierChip` and the share banner. 6 unit tests cover not-complete → null, inferred-matches → null, render path, switch updates store, stay writes 30-day expiry, active suppression suppresses re-render. (c) **Hidden-by-role footer on Report bottom** [view:/report] — small bordered footer surfaces sections the current persona is hiding from the summary view (`getReportSectionConfig(persona, id).state === 'hidden'`), listing them by `REPORT_SECTION_LABELS` in italic so the user knows exactly what's collapsed without scrolling back to the top toggle banner. One-click "Show full report" expands. Light refactor lifted the section-id list into a `REPORT_SECTION_ORDER` constant so the mode-detection and the footer share one source of truth. (d) **BC hidden-by-lens transparency footer** [view:/business] — small text footer at the bottom of the Business Center body names the zone the active persona emphasizes and how many of the six CSWP.39 zones get curated artifact featuring (e.g. "Your developer lens emphasizes the migration zone and a curated artifact set (3 of 6 zones featured)"). Renders only when a persona has its own emphasis config. Acts as a soft invite to switch role via the chip in the page header. (e) **Audit closeouts** — verified done via existing infrastructure (no new code needed) and recorded in commit bodies: P15-P1-01 (section nav with scroll-spy — `ReportToc.tsx` already implements sticky right rail + IntersectionObserver + mobile selector + smooth-scroll), P15-P1-06 (persona-aware CTA list — `PERSONA_REPORT_CTAS` already wired at `ReportContent.tsx:1527`), P13-P1-03 (wizard preview card — existing `ModeSelector` already previews modes + ETA before the wizard mounts), P12-P1-01 (Stack mode default — `useMigrateSelectionStore` default is already `'stack'`), P12-P1-03 (persona switch chip + share in Migrate header — PageHeader → PersonaChip + `shareTitle`/`shareText` already deliver both), P16-P1-01 (`ExploreView.tsx:92` already unifies selectedPersona + experienceLevel curious detection), P16-P1-02 (`TILES[].recommendedFor` already persona-aware; CC-17 CuriousGuide step 4 deep-links to `/explore`), P05-P1-01 (`Dashboard.tsx:495` `resumeModule` already renders an up-next hero), P05-P1-02 (`Dashboard.tsx:1116` already has collapsed advanced filters), P03-P1-01 ("View Top 5" executive deep-link already at `AlgorithmsView.tsx:506-525`), P03-P1-02 (curious-preview block at `AlgorithmsView.tsx:537-575` already explains the 3 NIST algorithms in plain English), P17-P1-01 (`PatentsView.tsx:46-93` already has essential/algorithms/full/custom column-picker presets with localStorage persistence), P17-P1-03 (`PatentsTable.tsx:516,692` already calls `logPatentView` on row open — gained persona dimension via the earlier `personaLabel()` wrap), P14-P1-05 (PageHeader pattern + share props already deliver both), P14-P1-06 (curious `PreviewBanner` already mounted via P14-P0-1). Deferred as net-new design / heavy layout / not-applicable: P15-P1-07 (Curious glossary auto-link — renderer pass), P11-P1-03 (regulator left-rail — biggest layout refactor in plan), P14-P1-02 (persona KPI hero — design scope), P14-P1-03 (BC zone sidebar refactor), P14-P1-07 (mobile KPI compact — nothing to compact today), P12-P1-02/04/05 (Curious Migrate mode + glossary + hidden-by-role footer — need new persona column-filter logic and glossary infra), P18-P1-01/02 (About outbound link wrapping — touches 10+ sub-section files), P01-P1-02 (maturity tier card refactor — replace belt rank), P13-P1-06 (Curious 5-Q wizard variant), P06-P1-01..04 (Module page touches — recommended-next CTA, persona events, intro card, glossary auto-link), P04-P1-02 (Curious library picks — data file), P07-P1-02 (Curious playground descriptions — data file). Total Phase 1 unit-test count: 57 new tests. Phase 1 is substantively complete; remaining 10-12 tasks are heavy redesigns or low-leverage renderer passes.

- **Business + Curious UX — Phase 1 per-page round 2** [persona:executive][persona:developer][persona:architect][persona:ops][persona:researcher][persona:curious][view:/compliance][view:/assess]: Eight focused per-page integrations across Compliance and Assess. (a) **Persona-aware compliance tabs** [view:/compliance] — `PERSONA_COMPLIANCE_TABS` config + `getComplianceTabOrder` / `getComplianceOverflowTabs` helpers cap the TabsList at 3 primary tabs for every persona except researcher, who still sees all 6 inline. Each persona's primary set is `foryou`-first then the two highest-signal tabs for that role (executive: foryou · landscape · records; architect: foryou · cswp39 · landscape; developer: foryou · landscape · cswp39; ops: foryou · records · cswp39; curious: foryou · records · landscape). `MoreTabsMenu` gained a generic `tabs?` prop driven by a single `TAB_META` table so overflow tabs render with the right icon + label. (b) **Architect / developer / ops industry hints** [view:/assess] — `PERSONA_INDUSTRY_STEP_HINTS` gains 11 new finance + healthcare overrides for the three previously-empty technical personas across `compliance`, `sensitivity`, `use-cases`, and `retention` step keys; developer hints flag library + crypto-module obligations + JOSE migration paths, architect hints call out SWIFT + card-HSM migration order and device-PKI bottlenecks, ops hints map cert-fleet renewal-window-first ordering and HSM partition rotation. (c) **"Why we ask" inline rationale per wizard step** [view:/assess] — new `WIZARD_STEP_RATIONALE` map carries a one-line plain-English explanation of why each of the 14 assess steps matters. New `<WhyWeAskHint>` component (`src/components/Assess/steps/WhyWeAskHint.tsx`) renders the rationale inline for curious viewers (selectedPersona === 'curious' || experienceLevel === 'curious') and behind a small "Why we ask" disclosure pill for every other persona; wired into 5 step files (Step6Migration, Step7UseCases, Step10CryptoAgility, Step13TimelinePressure, StepCredentialLifetime). (d) **Curious "About this regulation" prefaces** [view:/compliance] — `src/data/complianceCuriousPrefaces.ts` carries one-paragraph plain-English explainers for 12 of the most user-facing frameworks (NIST, ENISA, BSI, ANSSI, FIPS, CNSA-2, DORA, NIS2, PCI-DSS, HIPAA, SOX, GDPR). `FrameworkDetailPopover` renders the preface in a primary-tinted callout above the technical description when persona is curious. (e) **Persona-aware framework emphasis** [view:/compliance] — `PERSONA_COMPLIANCE_FRAMEWORK_EMPHASIS` names the 5-6 most-relevant frameworks per persona; `FrameworkCard` gains a soft primary-tinted border + "For you" pill for emphasized frameworks. Non-filtering — every framework still renders in the grid. (f) **CI gate wire-up CTA** [view:/compliance][persona:developer] — new `CIGateSnippet` section in `DeveloperImplementationView` renders a GitHub Actions YAML scaffold that runs `@cyclonedx/cdxgen` + `jq` to fail builds with classical-only crypto; copy button with clipboard + `Compliance · CI Gate Copied` telemetry. (g) **ValidationGantt deadline timeline** [view:/compliance] — new `src/components/Compliance/ValidationGantt.tsx` plots applicable frameworks as horizontal bars from "now" to their deadline year so executives + architects can see relative regulator pressure at one glance. Bars are color-coded by `deadlinePhase` (active / imminent / near / mid / long / ongoing); caps at 12 bars by default; surfaces undated frameworks in the legend footer; mounted in `ExecutiveTimelineView` between RegulatoryClock and the tier framework grid. (h) **Module-level certification status panel** [view:/compliance][persona:developer] — new `ModuleCertificationStatus` four-tile dashboard (Modules tracked · With PQC support + % · ML-KEM validated · ML-DSA validated) computed client-side from the `useComplianceRefresh` records cache, with a footer line naming how many modules still ship classical-only crypto and a deep-link into the Records tab; mounted in `DeveloperImplementationView` between the Implementation Jump Bar and the CI Gate snippet. (i) Phase-1 round-2 ships 21 new unit tests (compliance tabs 5, framework emphasis 4, WhyWeAskHint 6, ValidationGantt 6, ModuleCertificationStatus 4) on top of the round-1 23 — total 44 new Phase-1 unit tests; every affected suite stays green. P11-P1-03 (one-regulator-at-a-time left-rail) deferred as its own focused session — biggest layout refactor in the plan.

- **Business + Curious UX — Phase 1 shared toolkit + per-page surface** [persona:executive][persona:curious]: Five shared components and three page-level integrations that close the density-relief gap. (a) New `<ResumeBanner>` (`src/components/common/ResumeBanner.tsx`) reads `useModuleStore.modules`, picks the most-recently-visited module with `status !== 'not-started'`, renders a "Continue X" link with per-route session-dismiss + `Resume Banner · Shown / Dismissed / Resume Clicked` telemetry; mounted on Landing replacing the inline implementation, ready to consume on Report and Business Center. (b) `PERSONA_BELT_TIER_LABELS` + `getBeltTierLabel()` helper in `personaConfig.ts` overlay the seven generic belts (White→Black) with four persona-relevant tiers — Executive sees "Briefed → Aligned → Sponsoring → Board-Ready", Curious sees "Aware → Informed → Confident → Quantum-Native"; rendered as a small suffix next to the belt name in `Landing/ScoreCard.tsx` and as a chip below the Report `PageHeader`. (c) Three new curious-only achievements — `first-jargon-decoded` (Glossary opened), `first-standard-read` (first Library item open), `met-the-quantum-threat` (CuriousOrientationView mounted) — each driven by a `curious:*` sentinel pushed to `sectionsVisited`, gated against the five other personas via `PERSONA_EXCLUDED_ACHIEVEMENTS`. (d) New `<TopThreeActions>` (`src/components/common/TopThreeActions.tsx`) caps action surfacing to 3 glass-panel cards with `Top Three Actions · Action Clicked` telemetry carrying `{source}:{position}:{id}` so we can slice by surface and by card position; mounted on Report top above the CSWP.39 nav with `result.recommendedActions.slice(0,3)` and category-flavored descriptions ("Immediate · do now" / "Short-term · this quarter" / "Long-term · plan it"). (e) New `<CuriousGuide>` (`src/components/common/CuriousGuide.tsx`) — 4-step floating tour bottom-right of Landing for first Curious visit, steps "Everything's encrypted" → "Quantum changes the math" → "The clock is already ticking" → "Find your starting point", each with a CTA into `/learn/pqc-101` | `/threats` | `/timeline` | `/explore`; dismiss persists via `usePersonaStore` v6 → v7 migration adding `curiousGuideDismissed`. (f) Phase 1 ships 23 new unit tests across the 5 shared components (PersonaChip 3, PreviewBanner 6, useUrlPersonaOverride 4, kpiCatalog 1, personaConfig 6, ResumeBanner 5, TopThreeActions 6, CuriousGuide 6); every affected suite stays green.

- **Three new persona-specific Compliance "For You" views** [persona:developer][persona:ops][persona:curious][view:/compliance]: All six personas now have dedicated For You content. New `DeveloperImplementationView` renders an Algorithm Coverage strip (PQ families that appear across your applicable standards, chips deep-link to `/algorithms?highlight=`), an Implementation Jump Bar (OpenSSL Studio · PKI Workshop · JOSE/JWT module), and a Standards→Implementation table cross-linking each framework to its library docs. `OpsRotationView` renders a Rotation Clock that buckets applicable frameworks by `deadlinePhase` (active / imminent / near / mid / long / ongoing), a Toolchain Jumps strip, and a Framework Deadlines table. `CuriousOrientationView` opens with a plain-English "What is compliance, in three minutes" card (rules → algorithms → deadlines) plus cross-links to PQC 101 / Threats / Timeline, followed by up to 5 applicable frameworks with friendly descriptions. All three reuse `useApplicabilityWithPaths` so the engine output matches the executive / architect / researcher views.
- **Persona-specific hero taglines on Landing** [view:/]: When a persona is selected on `/`, a second line renders under the shared hero body with role-specific urgency framing — boards / CNSA 2.0 deadlines for executives, library + JOSE shipping status for developers, hybrid certs + key-size jumps for architects, FIPS 203/204/205 + RFC 9964 + ACVP for researchers, cert rotation + HSM firmware for ops, and a plain-language "what is this" for curious explorers.
- **Curious-aware preview gate on /algorithms** [persona:curious][view:/algorithms]: When a curious user lands on `/algorithms` without a deep-link `?highlight=` parameter, the 146-row tech tables are replaced with a teaser card explaining the three NIST-selected algorithms (ML-KEM-768, ML-DSA-65, SLH-DSA-SHA2-128s) and a "Show full algorithm comparison" CTA that flips `viewAccess` to unlocked. Closes the prior bug where the preview banner sat above a fully-rendered tech table — defeating the gate's purpose.
- **"For me" persona filter on /changelog** [view:/changelog]: New filter chip beside the section type filters honours explicit `[persona:X]` tags on entries first, falls back to a per-persona keyword regex (e.g. compliance / regulatory / NIST for executive; API / SDK / library / JOSE for developer) so the filter is useful even on entries without explicit tags. Reduces the ~11,700-word changelog to the persona-relevant subset.
- **Crypto-libraries + crypto-automation chip selectors on /assess Step 11** [persona:developer][persona:ops][view:/assess]: Two new multi-select chip groups under the Infrastructure step — `cryptoLibraries` (OpenSSL, BoringSSL, wolfSSL, Mbed TLS, liboqs, GnuTLS, NSS, .NET, JCA/BouncyCastle, Go crypto, Rust crypto, PKCS#11) and `infraAutomation` (Terraform, Ansible, cert-manager, Vault, AWS KMS/CloudHSM, Azure Key Vault, GCP KMS, Smallstep, Manual, None). Persisted with a v1 → v2 schema bump on `useAssessmentFormStore` with safe-default migration.
- **Timeline persona hints for ops + curious** [persona:ops][persona:curious][view:/timeline]: `TIMELINE_PERSONA_HINTS` gains two new entries: ops gets "Watch the Deploy phase column — countries entering it set the certificate-rotation clock for your fleet," and curious gets "Each row is one country/region; bars show how far they have moved through five PQC migration phases."
- **Executive redirect banner on /openssl** [persona:executive][view:/openssl]: When an executive lands on the raw WASM workbench, an info banner now appears at the top with outline-button cross-links to `/compliance`, `/migrate`, and `/algorithms` so the page acknowledges executives may prefer higher-level surfaces.
- **Curious orientation card on /threats** [persona:curious][view:/threats]: `ThreatsDashboard.personaSummary` now has a curious branch that renders even without industries selected — "N known quantum-era threats — each one is a place where today's encryption could be broken once a large quantum computer exists. Pick an industry below to see the ones closest to you." First-content-is-avionics-jargon issue closed.
- **PERSONA_HERO_TAGLINE map on Landing** [view:/]: Six-entry table powering the new persona second-line copy. Keeps each tagline ≤ 22 words.
- **RFC 9964 — ML-DSA for JOSE and COSE** [view:/library][view:/algorithms][view:/learn/api-security-jwt]: Published May 2026 by the IETF COSE WG; supersedes `draft-ietf-cose-dilithium-11` (AUTH48-DONE). Registers JOSE `alg` values for ML-DSA-44/65/87, the new `kty="AKP"` (Algorithm Key Pair) key type (COSE kty=7), and the canonical 32-byte FIPS 204 seed encoding for private keys. Surfaced across the stack: new `library_05202026_r1.csv` row + cached `public/library/RFC_9964.html`; `concept_registry_05202026.csv` adds `standard:rfc-9964` with draft slug aliases; `concept_xwalks_05202026.csv` re-points 11 relationships from the draft to the RFC; Protocol Matrix COSE + JOSE rows upgraded to `stage: 'rfc-published'`; WebAuthn row's inherited reference updated. RAG corpus regenerated (2 new chunks for RFC-9964).
- **AKP JWK display in PQC JWT Signing workshop** [view:/learn/api-security-jwt]: New "View as JWK" toggle in the keypair card (Step 3) renders the generated ML-DSA keypair as an RFC 9964 §3 AKP JWK — `{kty:"AKP", alg, pub, priv:<32-byte-seed>, kid}`. To make the seed available, `generateJwsKeyPair` now generates its own 32-byte seed via `crypto.getRandomValues` and passes it to `ml_dsa.keygen(seed)` so noble's internally-discarded seed is preserved on the returned `JwsKeyPair` as `mldsaSeed`. New `toAkpJwk()` helper in `jwtUtils.ts` (ML-DSA only; returns `null` for SLH-DSA / composite / HSM-backed keys per RFC 9964 scope). Includes a one-line `HashML-DSA` scope-exclusion disclaimer (RFC 9964 §7.2 explicitly excludes HashML-DSA from JOSE/COSE).
- **Generic `EvidenceBadge` component** [view:/library][view:/migrate]: Domain-agnostic chip cluster (trust tier · freshness state · confidence score · cached-doc link) extracted from the Timeline-specific badge. Renders nothing when no signals are present. Used today on Library document cards (freshness chip + cached PDF link) and Migrate software cards (freshness chip from `lastVerifiedDate`). `TimelineEvidenceBadge` keeps its tier-vocabulary specialisation untouched. Source: `src/components/ui/EvidenceBadge.tsx` + tests.
- **`SourcePassagesDrawer` + `useSourcePassages` hook** [view:/library][view:/compliance][view:/migrate][view:/threats]: Collapsible per-record drawer that surfaces W3C PROV-DM source passages from the RAG corpus (`prov.source_passages[]`) inline on data cards. The hook resolves chunks via the existing `UnifiedSearchService` singleton (no second corpus fetch) and settles synchronously when the corpus is already loaded. Wired to Library, Compliance, Migrate, and Threats cards with the appropriate domain-prefixed chunk ids (`library-{referenceId}`, `compliance-{frameworkId}`, `software-{productId|slug}`, `threat-{threatId}`). Timeline deferred — chunk ids are opaque sequential indices and would need a generator-side stable-key refactor.
- **`prov?: ChunkProv` on `RAGChunk` type**: The PROV-DM block emitted by `generate-rag-corpus.ts` is now part of the TypeScript surface so downstream consumers can use the evidence chain without casting. Fields are optional in the type to accept partial / historical corpus shapes.
...
- **QRNG Demo: `WhyThisMatters` — statistical equivalence vs. quantum origin** [view:/playground]: New collapsible callout in PT-012 explains why CSPRNG and QRNG output are statistically indistinguishable under every standard test (monobit, frequency, min-entropy, autocorrelation), when a CSPRNG is fully sufficient (NIST FIPS 140-3 approved DRBGs for session keys and nonces), and exactly when QRNG matters: long-lived key material in high-assurance HSMs where seed unpredictability must be provable, and quantum-resistant long-term key generation. Notes that NIST SP 800-90B entropy source certification requires more than statistical tests alone.
- **PKI Enrollment (PT-029): copy-to-clipboard on generated key and issued certificate** [view:/playground]: `KeyGenStep` inspector now uses `CopyableOutput` for the private key PEM (copy + `.pem` download). `CmpInitialReq` result panel replaces both raw `<pre>` blocks — the decoded `openssl x509 -text` transcript and the issued cert PEM — with `CopyableOutput`; the cert PEM block includes `downloadFilename="issued-cert.pem"` so users can save it directly for use with `openssl verify` or import into a trust store.
- **`WhyThisMatters` shared component** [view:/playground][view:/learn]: Collapsible educational callout with `info` / `warning` / `highlight` variants. Explains the cryptographic and security significance of a tool step — why the operation exists, how it maps to FIPS / RFC constraints, and what a quantum adversary changes. Wired into TPM (TPM hierarchy + `CKA_EXTRACTABLE=FALSE` hardware-bound keys), MLS (Double-Ratchet → TreeKEM group PCS), TLS Simulator (authentication vs KEX separation, why half-migration is insufficient), QKD Key Derivation (SP 800-56C domain separation, KBKDF counter format), and DigitalID Relying Party (OpenID4VP selective disclosure + Key Binding proof path).
- **`ScopeTransparencyBanner` shared component** [view:/playground]: Two-column real-ops vs. simulated-ops disclosure panel. Each column lists exactly what the tool executes via real crypto vs. what it illustrates through simulation. Wired into QKD Key Derivation (ETSI QKD 014 REST retrieval simulated; `C_CreateObject` + `C_DeriveKey` real) and TEE–HSM Trusted Channel (ML-DSA-65 attestation, ML-KEM-768 encapsulation, AES-256 key wrap real; TEE quote format, topology diagram simulated).
- **`CopyableOutput` shared component** [view:/playground]: Read-only `<Textarea>` with one-click copy and optional file download. Wraps raw hex, PEM, and JWT token outputs so users can inspect, paste, or save to disk. Wired into DigitalID VP presentation token + Key Binding Proof outputs (Sprint C) and DRBG generated output (Sprint D).
- **Hybrid Certificates (PT-016): `WhyThisMatters` — hybrid cert migration rationale** [view:/playground]: New collapsible educational callout in the Hybrid Certificate Formats workshop step explains why hybrid certificates allow PQC migration without breaking existing relying parties. Covers harvest-now/decrypt-later threat timeline, three deployment approaches (dual-cert, catalyst/chameleon, and LAMPS composite per `draft-ietf-lamps-pq-composite-sigs`), and when each approach applies based on relying-party upgrade pace.
- **TLS 1.3 Simulator (PT-024): per-flight RFC 8446 §4 annotations** [view:/playground]: `CryptoLogDisplay` now renders a concise RFC 8446 §4 citation beneath each handshake message type — ClientHello §4.1.2, ServerHello §4.1.3, EncryptedExtensions §4.3.1, Certificate §4.4.2, CertificateVerify §4.4.3, Finished §4.4.4, NewSessionTicket §4.6.1, CertificateRequest §4.3.2, and KeyUpdate §4.6.3. Gives developers the exact spec section to cross-reference without leaving the tool.
- **PKI Enrollment (PT-029): steps 3–4 in playground view** [view:/playground]: `PKIEnrollmentPlayground` now renders the full four-step enrollment flow — Step 3 (`EstSimpleEnroll`, RFC 7030 simpleenroll) and Step 4 (`CmpKemKeyUpdate`, RFC 9810 encrCert POP) wired alongside the existing Steps 1–2. Intro banner updated to "Four-step PKI enrollment showcase — RFC 4210 (CMP) · RFC 7030 (EST) · RFC 9810 (CMP KEM)". Previously only two of the four available steps were shown in the playground view.
- **OpenSSL Studio (PT-023): copy-all and per-line clipboard in terminal output** [view:/openssl]: `TerminalOutput` toolbar gains a "Copy all" button that copies all visible filtered log lines joined by newlines. Each log row exposes a `Copy` icon on hover (`opacity-0` → `opacity-100` via Tailwind group-hover) that copies the single line with a 1.5 s confirmation. Fills the gap where multi-line `openssl` output (PEM blocks, speed-test results) could not be copied out of the terminal.

### Fixed

- **A7 progress-bar sweep — 12 workshop surfaces + Algorithms benchmark + shared `KatValidationPanel` + `useStepWizard` hook** (2026-05-22, local-only): Builds on the `WorkshopOperationLog` indeterminate progress bar that landed in PR #241. Where the original A7 wave covered 9 EmailSigning + PKIEnrollmentProtocols components, this sweep extends the pattern to every remaining truly-slow surface in the app — the genuine "blank pane for many seconds" symptom. (a) **`AlgorithmComparisonPanel` live benchmark** [view:/algorithms] — the per-algorithm benchmark loops up to 12 algorithms × 10 ops = 120 sequential WASM ops; each iteration now pushes a `${algo} — run i/N` log entry resolving to `keyGen Xms · sign Y ops/s · verify Z ops/s` on success or the exception message on failure. (b) **`LMSKeyGenDemo`** + **`XMSSKeyGenDemo`** [view:/learn] — stateful keygen at high tree heights (LMS H25 ~60s, XMSS H20 ~10min by `XMSS_KEYGEN_MS` declaration) now shows a single pending entry until completion; existing duration-paced strips kept. Bespoke `AlertCircle` error blocks replaced with `<ErrorAlert onRetry={...}>`. (c) **`TokenSizeAnalyzer`** [view:/learn] — on-mount sign benchmark across all measurable JOSE PQC sig algorithms (ML-DSA-44/65/87, SLH-DSA-SHA2-128s/192s/256s, composite ML-DSA-65-Ed25519) now appends one entry per algorithm `Measured ML-DSA-65 — 3,293 bytes`. (d) **`KatValidationPanel`** (shared component) [view:/learn] — biggest leverage hit because this panel is mounted across 5+ workshops: `KmsPqc/EnvelopeEncryptionDemo`, `FiveG/SuciFlow`, `Healthcare/PatientPrivacyMapper`, `ConfidentialComputing/AttestationWorkshop`, `DatabaseEncryption/BYOKKeyDesigner`. The shared `runKATs` async path now wraps the HSM init + each KAT spec with `beginOp` log entries (e.g. `Initializing softhsmv3 HSM module…`, `Running ML-KEM-768 key encapsulation via PKCS#11…`); custom `AlertCircle` error banner replaced with `<ErrorAlert onRetry={runKATs}>`. Closes the "ACVP testing into PKCS#11 HSM takes minutes" symptom. (e) **`HybridKeyGeneration`** [view:/learn] — `generateAll()` parallel keygen loop now appends `Generating ${algo} keypair…` log entries per algorithm; existing per-card timingMs + error rendering kept. (f) **`TEEHSMTrustedChannel`** [view:/learn] — multi-step TEE↔HSM provisioning flow (PQC + classical paths) now shows one overall pending entry + per-step success entries inserted as `addResult()` fires. ErrorAlert gains `onRetry`. (g) **`SLHDSALiveDemo`** [view:/learn] — shared `runOp` helper wraps the keygen + sign + verify handlers; SLH-DSA-SHA2-256s sign is 1-3s on slow machines. (h) **`HybridSignatures`** [view:/learn] — shared `beginOp` helper covers the 3-construction (concatenation / nesting / silithium) keygen + sign + verify matrix; construction id is in the log message for cross-tab correlation. (i) **`FirmwareSigningMigrator`** [view:/learn] — `handleSignBoth()` dual-sign (classical + PQC sequential) gets a single overall pending → success entry with durationMs. (j) **`StatefulSignaturesDemo`** [view:/learn] — `handleVerify()` cross-engine signature verification (Rust signature → C++ engine) gets a pending → success/error entry; `verifyError` ErrorAlert gains `onRetry`. (k) **`useStepWizard` shared hook** — adds `logEntries` state surfaced on the hook's return value, fed by per-step pending → success/error entries. 8 consumers (`HybridEncryptionDemo` + 3 FiveG flows + 4 DigitalAssets flows) opt in by rendering `<WorkshopOperationLog entries={wizard.logEntries} />`. `HybridEncryptionDemo` opts in now; the other 7 get the data exposed but haven't rendered yet. (l) Skipped on purpose (sub-100ms, no perceived symptom): `JWTInspector` (single verifyJWS with in-button Loader2), `Entropy/SourceCombiningDemo` (HSM KDF chain — all sub-second), QKD pair `PostProcessingDemo` + `HSMKeyDerivationDemo` (already paced for educational visibility via `setTimeout(400)`). The A7 audit now reads PARTIAL with ~28 lower-impact surfaces remaining; see `pqctoday-priv/docs/platform/ux/ux-gap-analysis.md` for the full coverage map.
- **`ReportView` perf — `computeAssessment` was running on every render** [view:/report]: The 10-stage scoring pipeline (algorithm migration mapping → industry/country compliance filter → 4 category scorers → composite-with-boosts → HNDL/HNFL windows → migration effort → category drivers → recommended actions) was called inline at `ReportView.tsx:102` without `useMemo`. Any persona toggle, any module-store update, or any unrelated remount triggered a full recompute. Fix: `useMemo` keyed on `JSON.stringify(input)` — `getInput()` returns a fresh `AssessmentInput` reference each store read, so plain `[input]` deps would never hit; the stringify is ~30 small primitive/array fields, cheap relative to the pipeline. On slow machines: report becomes interactive in ~50ms instead of 800-1500ms after persona switches / unrelated updates. Initial first-mount compute is unchanged. 31/31 Report tests still pass (pure memoization change, no semantic change to the computed result).
- **`ux-gap-analysis.md` refreshed to match May 22 reality** (private repo): A5 reopened, A8, A9 marked **RESOLVED** — they were already closed in code but the audit doc was stale. A8 closed by `<MotionConfig reducedMotion="user">` wrapping the entire app at `AppRoot.tsx:25` (every Framer Motion component automatically respects `prefers-reduced-motion`); the originally-proposed custom `usePrefersReducedMotion` hook is superseded by the Framer primitive. A9 closed by `781d6d24` + `8c043ed2` (11 + 41 modules wired). A7 marked PARTIAL with the 22-surface migration roster + cross-referenced commit SHAs. New tracked **A5b** entry covers 11 native `<select>` elements outside the original Learn-UI A5 scope (`AlgorithmComparison`, `WorkshopPanel`, `ReportToc`, `CommandBuilder`, `VpnSimulationPanel`) — real S4.3 violations, not user-affecting enough to block other work.
- **CI hardening — four blockers cleared on the Business + Curious UX branch** (2026-05-21): (a) **TP-1 dropped from CI** (`.github/workflows/ci.yml` step "Validate threats proof rule") — the rule asserted every active `quantum_threats_hsm_industries_*.csv` row's `local_file` must exist on disk, but `.gitignore:280-281` excludes `public/threats/*.pdf` and `public/threats/*.html` so clean-checkout CI was deterministically false-positive. Verified the threats UI consumes nothing from those files (`transformThreat()` in `src/data/threatsData.ts` drops the column; zero references in `src/components/Threats/*`) before removing. TP-2 (path under `public/threats/`) and TP-3 (catalog id resolution) are kept — they don't depend on disk state. The TP-1 validator function remains in `scripts/validators/threats-proof-rule.ts` for local dev. (b) **Trust-engine attestation step guarded behind `hashFiles()`** — `scripts/attestation/verify-all.ts` lives under the gitignored `scripts/*` block; the verifier ran on every push and hard-failed when the source wasn't checked out. Now skipped when `scripts/attestation/verify-all.ts` is absent; flips back to hard-fail the moment it lands in-tree. (c) **`tsc -b` (build mode) strict-typing pass** — six TS errors only `tsc -b` surfaces (project-references walk) but `tsc --noEmit` misses: Blob ctor in `MLDSASignDemo` wrapping a `Uint8Array<ArrayBufferLike>` (slice to materialise a non-shared buffer), test-mock override unions in `PersonaSuggestionCard.test.tsx` + `ResumeBanner.test.tsx`, `ComplianceTabId`/`MobileSection` boundary cast in `ComplianceView.tsx`, `bodyType` literal `'regulator'` → `'regulatory_body'` in two FrameworkFocusView/ValidationGantt tests, `ApplicabilityResult<ComplianceFramework>[]` → `ComplianceFramework[]` unwrap on the ValidationGantt mount in `ExecutiveTimelineView.tsx`. (d) **`corpus-trust-invariants` tier-gap ceiling bumped 107 → 113** — threats Phase 4-8 recovery commits (`83d370fe`, `980496ee`, `6046d41c`) emitted 6 new `document-enrichment` chunks (RFC 7030, RFC 7296, `draft-reddy-cose-jose-pqc-hybrid-hpke`, etc.) without trust-score wiring; same gap as the prior 5 documented bumps. Drive back down via `trustScoreData.ts:chunkToResource` extension later. Together these four fixes brought the Business + Curious UX PR from CI-red to CI-green without touching feature scope.
- **/learn missing h1 — WCAG 2.4.6 / 1.3.1 (closes BLOCKED finding)** [view:/learn]: The Learn Dashboard rendered no `<h1>` element across any of the 6 personas, violating WCAG Headings and Labels + Info and Relationships. The "Learning Workshops" title at `Dashboard.tsx:930` was promoted from `<h2>` to `<h1>`. Each module page (PKIWorkshop, TLSBasics, PQC101, CuriousModuleView) already renders its own h1, so the fix is scoped to the dashboard view only.
- **Gap A9 — WorkshopStepper labels missing across 52 PKILearning modules** [view:/learn]: Every workshop index file used to render `<WorkshopStepHeader>` without the optional `steps` / `completedSteps` / `onStepClick` props, so users navigating multi-step workshops saw anonymous "Step 1, Step 2, Step 3" dots and could not click back to a completed step. All 52 modules now pass `steps={PARTS.map(p => ({ id: p.id, label: p.title }))}` and `onStepClick={handlePartChange}` (or `handleStepChange` / `setCurrentStep` depending on the module's state setter). `EmailSigning` additionally drops its pre-WorkshopStepper inline custom dot-stepper in favour of the shared component — that custom stepper was the audit's call-out "reference migration." Special cases: `CryptoMgmtModernization` keeps its `cswp39Step` prop alongside the new ones; `Module1-Introduction` uses persona-filtered `visibleParts` rather than `PARTS`; `PKIEnrollmentProtocols` uses `WORKSHOP_STEPS` / `handleStepChange`. Test fix: `QKD.test.tsx` "switches to Workshop tab" expected exactly one `Part 1: BB84 Protocol` element — labelled stepper now renders the title in both h2 + dot pill, so the assertion switched to `getAllByText`.
- **Gap A7 — async loading + error/retry states missing in 9 workshop crypto demos** [view:/learn]: Every WASM-driving workshop component in PKILearning now renders `<WorkshopOperationLog>` (per `ux-standard.md §S4.13`) with `performance.now()` duration per stage, plus `<ErrorAlert message={error} onRetry={run} />` with a Retry button. Components migrated: EmailSigning trio — `MLDSASignDemo` (genkey → mkcert → sign → verify, 4 stages), `MLKEMEncryptDemo` (ca-key → ca-cert → genkey → mkcert → encrypt → decrypt, 6 stages), `DualSignDemo` (pq-key → cl-key → pq-cert → cl-cert → sign → verify, 6 stages); PKIEnrollmentProtocols six — `KeyGenStep`, `CertViewer`, `EstSimpleEnroll`, `CmpInitialReq`, `CmpKemKeyUpdate`, `CompositeEnroll`. `RandomGenerationDemo` retains its per-source contextual loading/error layout (parallel-generators pattern). `OpenSSL Studio` keeps its existing `TerminalOutput.tsx` — it is structurally a live operation log already.
- **/algorithms Protocol Support tab "WIP" badge replaced with "Beta"** [view:/algorithms]: The warning-coloured WIP chip on the Protocol Support tab was misleading after RFC 9964 adoption + the 11→8-column layout fix — the data is substantive. Replaced with a primary-coloured Beta chip and accurate hover text ("Tracks 14 IETF protocols across pure-KEM, hybrid-KEM, pure-Sig, hybrid-Sig dimensions. Updated weekly from datatracker.").
- **/migrate "WIP" badge replaced with "Community" framing** [view:/migrate]: The warning-coloured WIP chip on the Migrate context strip was the audit's "trust-destroying label on the freshest, most maintained dataset." Replaced with a primary-coloured Community chip + `Users` icon, and the disclaimer prose reframed from "data is being actively reviewed and validated" to "PQC product data is community-curated and updated continuously."
- **/library "Recent Updates" surfaces dataset refresh timestamp** [view:/library]: The audit flagged that `ActivityFeed` showed Nov 2024 dates on items despite a fresh May 2026 CSV — the dates were the documents' own publication dates, not the data freshness. Header now shows "Dataset refreshed `<libraryMetadata.lastUpdate>`" on the right, and per-card dates are relabelled `Pub. <date>` with hover clarification "Document publication or revision date."
- **/library category pre-selection from persona on mount** [view:/library]: `PERSONA_LIBRARY_CATEGORIES` already existed but was never applied at mount — only used as a secondary sort boost. `activeCategory` now initialises from `PERSONA_LIBRARY_CATEGORIES[selectedPersona]?.[0]` when no `?cat=` URL param is set, so each persona lands on a relevant slice (executive → Government & Policy, developer → Protocols, architect/ops → PKI Certificate Management, curious → Migration Guidance, researcher → All).
- **/about Vision now renders before Release Notes** [view:/about]: `AboutView.tsx` swaps the section order so first-time visitors land on the mission framing before the changelog entries. Closes the audit's "wrong entry point for evaluators" finding.
- **/changelog PERSONA_CONFIG missing ops + curious badge entries** [view:/changelog]: The post-resolution audit flagged that the new "For me" filter regex covered all 6 personas but the impact-badge map only had 4 (executive, developer, architect, researcher). `[persona:ops]` and `[persona:curious]` tags filtered correctly but rendered no styled badge in the timeline view. Added Wrench (ops) and Compass (curious) entries.
- **Playground CuriousStartHere "three steps" copy now ships three steps** [view:/playground]: The curious guided-start panel promised a three-step path but rendered exactly one (QRNG Demo). Now ships a coherent narrative: 1) See where randomness comes from (qrng-demo) → 2) Watch a website handshake (tls-simulator) → 3) Try hybrid (old + new) encryption (hybrid-encrypt).
- **ops persona nav path gains /leaders** [persona:ops][view:/leaders]: `ops` was the highest-traffic persona with zero nav path to the community page despite tool-maintainer relevance. `personaConfig.ts` `PERSONA_NAV_PATHS.ops` now includes `/leaders`. (The matrix was already showing it as a recommended add; this lands the code.)
- **Corpus PROV-DM `was_derived_from` populated for 97 in-code chunks**: `processNiceFramework`, `processProtocolMatrix`, the six regulatory-timeline emitters, and `processFrameworkFines` in `scripts/generate-rag-corpus.ts` all called `buildChunkProv({ attributedTo: 'human' })` with no `csvFile` and no `enrichmentFile`, causing `was_derived_from` to fall through to an empty string. The corpus-trust-invariants PROV-DM test (`every chunk has prov with the 4 always-required PROV-DM fields`) was failing for 97 chunks across NICE (70), Protocol Matrix (27), regulatory-timelines (6), and framework-fines (1) sources. Fix: each emitter now passes an `enrichmentFile` pointer back to the in-code TypeScript constant it derived from (e.g. `src/data/niceFramework.ts:NICE_COMPETENCY_AREAS.{id}`, `src/data/pqcProtocolMatrix.ts:PROTOCOL_MATRIX[id={id}]`, `src/data/regulatoryTimelines.ts:{CONST_NAME}`). `sourceDocFile` intentionally left unset because in-code constants have no on-disk source file — adding one would break the separate `source_doc local paths resolve on disk` invariant. All 10 `corpus-trust-invariants.test.ts` tests pass.
- **API Security & JWT module spec citations updated to RFC 9964**: `content.ts` `relatedStandards`, `APISecurityIntroduction.tsx`, `constants.ts` (4 `spec:` fields + header comment + sample JWT comment), `jwtUtils.ts` (header + Appendix A.1 docstrings), `PQCJWTSigning.tsx` (link href + display text + footnote), `JWTInspector.tsx` (AKP seed attribution), `JOSEProtocolMatrixAudit.tsx` (KAT spec strings + reference URLs + audit-patch delta now emits `ref_id: 'RFC 9964'`, `current_stage: 'rfc'`), `APISecurityJWTPlayground.tsx` (Step 6 description). The KAT JSON file `cose-dilithium-11-jose-kat.json` keeps its name — vectors are byte-equivalent to RFC 9964 Appendix A.1, so renaming would force unnecessary import-path churn.
- **Performance & Size Considerations field corrected in RFC-9964 enrichment**: qwen3.6:27b extraction labeled the ML-DSA secret-key sizes (2560/4032/4896) as "Public Key" and the public-key sizes as "Signature". Hand-corrected `library_doc_enrichments_05202026.md` row to match FIPS 204 Table 1 (pk = 1312/1952/2592, sig = 2420/3309/4627, seed = 32 B). Provenance note appended inline so the field is visibly maintainer-touched.
- **Corpus trust invariants C3 / C4 / C5 enforced in CI** (closes audit gap V2): `src/__tests__/corpus-trust-invariants.test.ts` was previously skipped wholesale when `process.env.CI` was set. The 7 pure-check sub-tests (C3 tier coverage × 2, C4 PROV-DM completeness × 4, C5 freshness × 1) now always run — they don't depend on the gitignored reference-doc cache (the `source_doc` test already exempts `LOCAL_CACHE_SOURCES`) and they catch real drift on every PR. The 3 T16 embedding-coverage tests stay maintainer-local via a new `describeMaintainerLocal` gate because the corpusHash check requires the embeddings pipeline to be re-run in lockstep with each corpus change — a manual operation. Under simulated CI: 7 pass, 3 skip; locally: all 10 run.
- **Trust-engine attestation verified in CI** (closes audit gap P4): New `.github/workflows/ci.yml` step `Verify trust-engine attestations` runs `npm run trust-engine:verify` after the audit/lint pass and before the build, hard-failing on any signature mismatch, missing `.sig` file, or unknown kid. All 8 trust artifacts (revisions log, RAG corpus, OSCAL combined / governance / full / assessment plan, CBOM, community signals) re-signed at HEAD with kid `11b723084d047b4c` so the gate flips green from day one. Closes the audit gap where corrupted or tampered attestations could merge to main undetected.
- **Corpus trust C3 tier-coverage invariant fully passes**: Two-part fix in `chunkToResource.ts` + `src/__tests__/corpus-trust-invariants.test.ts`. (1) Compliance summary chunks without `metadata.id` (e.g. `framework-fines-summary` from `frameworkFines.ts`) now route to "unknown trust" instead of falling back to `chunk.title` and orphaning against `trustScoreData`. (2) Ten new aggregate / derived sources from the 2026-05-19 RAG corpus gap-fix (`nice`, `protocol-matrix`, `concept-xwalk`, `algo-product-xref`, `vendor-roadmap`, `implementation-attacks`, `concept-registry`, `regulatory-timeline`, `standard-algo-xref`, `counter-claims`) added to `TIER_NOT_APPLICABLE` — each summarises multiple underlying records rather than representing a single scoreable resource. Both C3 sub-tests (source coverage + tier resolution within pinned gaps) now green locally.
- **a11y: `jsx-a11y/label-has-associated-control` in three S/MIME workshop components** [view:/learn]: `MLDSASignDemo`, `MLKEMEncryptDemo`, and `DualSignDemo` all used a custom `<Textarea>` component which `jsx-a11y` does not recognise as a native control. Fixed by adding explicit `id` props to each `<Textarea>` and matching `htmlFor` on the associated `<label>`. Same pattern applied to `<Switch>` in `HsmCapacityCalculator` — the surrounding `<label>` was converted to a `<div>` because `<Switch>` renders a `<button>`, not a form input; the `aria-label` on the Switch itself provides the accessible name.
- **`<input type="checkbox">` replaced with `<Switch>` in HSM Capacity Calculator** [view:/playground]: The raw HTML checkbox in the use-case enable/disable row was replaced with the design system `<Switch>` component (`onCheckedChange` callback, `aria-label`, `shrink-0`). Closes a raw-element design system violation.
- **`<input type="text">` replaced with `<Input>` in 5G SUCI Flow** [view:/playground]: The SUPI text field was a raw `<input>` with hand-rolled class names. Replaced with `<Input>` from the design system, preserving `aria-invalid`, `aria-describedby`, error-state border styling, and the `font-mono` class for the IMSI display.
...
- **Solana Transaction (PT-021): raw `div[role=checkbox]` → `<Switch>`** [view:/playground]: `SolanaFlow.tsx` replaced a `<div role="checkbox" aria-checked={...}>` with a bespoke `onKeyDown` handler with the design system `<Switch>` component (`checked`, `onCheckedChange`, `aria-label="Simulate invalid signature"`). Closes the last raw-element design system violation across all 33 playground tools.
- **S/MIME Workshop (PT-031): `.p7m` clipboard and download in `MLDSASignDemo`** [view:/learn]: After the hex dump details block, a `CopyButton` (base64-encodes the raw `Uint8Array` via `btoa(String.fromCharCode(...))`) and a download `<Button>` (Blob with `type: 'application/pkcs7-mime'`) appear when a signed `.p7m` is present. The S/MIME envelope preview produced by `smimeEnvelopeSigned()` is now rendered via `CopyableOutput` with `downloadFilename="signed-message.eml"`, replacing a raw `<pre>` that offered no copy affordance.
- **RAG corpus: 10 unregistered sources wired into search routing and boost tables**: `corpus-invariants.test.ts` was failing on three invariants — `SOURCE_LABELS`, `chunkToRoute`, and `INTENT_BOOSTS`/`PERSONA_BOOSTS` — for 10 sources added in the May 2026 RAG corpus gap-fix (`algo-product-xref`, `concept-registry`, `concept-xwalk`, `counter-claims`, `implementation-attacks`, `nice`, `protocol-matrix`, `regulatory-timeline`, `standard-algo-xref`, `vendor-roadmap`). Fixed by adding `SOURCE_LABELS` display labels, explicit `case` arms in `chunkToRoute()` with sensible fallback routes, and targeted boost multipliers in `INTENT_BOOSTS` (across `definition`, `comparison`, `catalog_lookup`, `recommendation`, `country_query`, `standard_query`, `general`) and `PERSONA_BOOSTS` (developer, executive, architect, researcher, ops). All 7 corpus-invariant tests now pass.
- **Threats data: TP-1 + TP-2 proof-rule CI gate** (closes threats accuracy audit 2026-05-21): New `scripts/validators/threats-proof-rule.ts` enforces two invariants on the latest `quantum_threats_hsm_industries_*.csv`. TP-1: every active row must carry a `local_file` that exists on disk and is ≥ 5 KB (catches 404 stubs and landing-page redirects). TP-2: `local_file` paths must resolve under `public/threats/` (prevents cross-source contamination). Wired into `validate-data-integrity.ts` and added as a blocking CI step in `.github/workflows/ci.yml`. Severity: ERROR — rows that fail either rule must be re-sourced or marked `status='deprecated'` before the CSV ships.

### Changed

- **Protocol Matrix COSE + JOSE + WebAuthn rows promoted to `rfc-published`** [view:/algorithms]: `pureSig` dimension upgraded from `value: 'draft'` / `stage: 'rfc-editor-queue'` ("AUTH48-DONE") to `value: 'rfc'` / `stage: 'rfc-published'`. Refs now point to `https://www.rfc-editor.org/rfc/rfc9964.html` (May 2026). RFC 9964 added to `latestRelease` for both COSE and JOSE rows; `draft-ietf-cose-dilithium-11` removed from `latestDraft`. WebAuthn pureSig ref (inherited via COSE algorithm IDs) updated to cite the RFC.
- **Library + concept registry deprecate draft slugs in favour of RFC 9964**: `draft-ietf-cose-dilithium` and `draft-ietf-cose-dilithium-05` rows in `library_*` and `concept_registry_*` now carry `status: 'deprecated'`, `deprecated_at: '2026-05-20'`, `deprecated_reason: 'superseded by RFC 9964 (published 2026-05)'`. New active rows added for RFC-9964 with cross-aliases so prior deep links still resolve via the registry. Concept-xwalks: 11 rows re-pointed from the draft slug to `standard:rfc-9964` (CSV-aware Python rewrite via `csv` module, evidence-text fields untouched).
- **`RevisionDrilldownPanel` reachable from every data record** [view:/library][view:/compliance][view:/migrate][view:/timeline][view:/threats][view:/algorithms]: The `onOpenDrilldown` callback on `ReviewedBadge` is now wired on all six data pages — clicking the reviewer chip opens a local in-page drilldown panel instead of forcing a navigate-away to `/revisions`. Each card manages its own panel open state; `useRevisions()` + `byRecord()` scopes history per entity. Previously only `ModuleCard` and `PlaygroundWorkshop` honoured the callback.
- **Trust + reviewer badge coverage gap closed** [view:/compliance][view:/algorithms]: `ReviewedBadge` added to `ComplianceLandscape` framework cards (was missing); `TrustScoreBadge` added to `AlgorithmComparison` and `AlgorithmDetailedComparison` rows alongside the existing `ReviewedBadge`. All six data pages (Library, Compliance, Migrate, Timeline, Threats, Algorithms) now render the same Trust Score + Reviewed badge cluster per record.
...
- **Inter-tool navigation sequences** [view:/playground]: A `WorkshopNavigationContext` + `PlaygroundNextStep` footer component enables structured learning paths within the Playground. `WorkshopToolsTab` injects `selectTool(id)` via context; tool components render a "Next in sequence" card at the bottom that jumps directly to the next tool without returning to the tool grid. Two sequences wired: **Entropy path** PT-010 → PT-011 → PT-013 → PT-014 → PT-012 (Random Generation → Entropy Testing → Source Combining → SP 800-90A DRBG → QRNG Demo); **PKI sizing trio** PT-015 → PT-025 → PT-026 (PKI Workshop → Cert Capacity Calculator → HSM Capacity Calculator).
- **Playground tools: copy-to-clipboard on raw hex and signature outputs** [view:/playground]: Five tools previously displayed hex and signature bytes in unselectably-small or overflowing `<pre>` / `<div>` blocks with no copy affordance. Each tool now exposes a `CopyButton` or `CopyableOutput` (copy + download) adjacent to the output: Random Generation (PT-010) — hex bytes per source card; Entropy Testing (PT-011) — sample data panel header; Source Combining (PT-013) — all `HexDisplay` instances now include a Copy button in their label row, covering source A/B, combined, conditioned, and expanded outputs; SP 800-90A DRBG (PT-014) — last-generated output block replaced with `CopyableOutput` with `.txt` download; Hybrid Signature Spectrums (PT-027) — individual copy buttons on EC component (R‖s) and ML-DSA component hex.
- **QKD Key Derivation: real `C_CreateObject` + `C_DeriveKey` wiring** [view:/playground]: When the HSM toggle is on, Step 2 (Import Master Secret) now calls `hsm_importGenericSecret` to create a real PKCS#11 generic-secret object and stores the returned handle in component state. Step 3 (Run KDF) reuses that handle via `hsm_kbkdf` (`C_DeriveKey` with `CKM_SP800_108_COUNTER_KDF`) when the handle is present, falling back to the software `sp800108CounterKDF` path when not. A "Live HSM — C_CreateObject returned CKR_OK" badge appears in the Step 2 UI when the live path was taken. The `ScopeTransparencyBanner` lists both real and simulated operations explicitly.
- **Protocol Matrix fits standard screen widths** [view:/algorithms]: Column count reduced from 11 → 8. (1) Removed the row-level **Latest Release** + **Latest Draft** columns — the same RFC / draft IDs are already surfaced inside each Pure KEM / Hybrid KEM / Pure Sig / Hybrid Sig cell via `DimensionRefChip` (now confirmed clickable to the authoritative `datatracker.ietf.org` source for all per-cell refs). (2) Merged **Open Source** + **Commercial** into a single **Libraries** column with color coding preserved (OSS chips primary/blue, commercial chips accent). (3) Capped KEM / Sig column widths at 180px (`align-top`, `min-w-0`) so ref chips wrap to additional lines instead of widening the table. Heatmap view unchanged in spirit — the merged Libraries cell shows both `AvailabilityBadge` count chips side-by-side.

### Data

- **Threats catalog refresh + proof audit** (2026-05-21): `quantum_threats_hsm_industries_05212026.csv` replaces `05192026`. Rows with unverifiable local proofs deprecated in-place per DS-series self-containment rule. Two new threat proof documents added: `CROSS-011.txt` (cross-sector quantum cryptography exposure, 11 KB) and `IOT-003.txt` (IoT device key management, 65 KB). Four source URLs corrected in `public/threats/manifest.json` (rtca.org → FAA cybersecurity page; CSA working-group → artifact deep-link; NIST IR 8547 IPD landing → direct PDF; SP 800-210 landing → direct PDF). Trust-tier snapshot re-run: threats tier counts updated (High 23→22, Low 68→15; 16 rows deprecated), total corpus 3403→3387.
- **Threats catalog Phase 4–8 recovery + LLM rewrite** (2026-05-21): `quantum_threats_hsm_industries_05212026_r2.csv` supersedes `_05212026.csv` after a five-phase recovery + editorial pass that lifted active rows from 82 → **106** and merged 55 of 57 audit corrections inline so threat descriptions read naturally instead of carrying appended `Correction (audit 2026-05-18, verdict=…)` sentences. **Phase 4 — Playwright headless-browser snapshot**: nine rows that defeated curl recovered by driving Chromium with a desktop UA + JS render against bot-gated US/AU government endpoints (CNSA 2.0 PDF ×4 → AERO-002 + GOV-001/002/005; UToronto smart-grid → ENERGY-003; GS1 → RETAIL-003; IEC etech → RAIL-005; FERC → WATER-005; IACR ePrint → IT-006). **Phase 5 — Wayback Machine recovery**: five additional rows fetched via `archive.org/wayback/available` + the raw `id_` snapshot form to bypass Wayback's UI banner wrapper (AUS-GOV-001, TELCO-002, TELCO-004, EDU-004, RAIL-004). **Phase 6 — manual re-source**: ten rows swapped to live-verified primary-source URLs (each probed via `curl GET` ≥ 70 KB content before commit): CROSS-001/003 → ibm.com/think PQC overview; TELCO-001 → 3GPP TS 33-series catalogue; CLOUD-001/002 → CSA quantum-safe hub; WATER-002 → CISA water sector; EDU-002 → InCommon; AUS-TEL-001 → legislation.gov.au CIRMP Rules 2023; PCI-002 → PCI-SSC document library; CRYPTO-002 → Ethereum.org account-abstraction roadmap. **Phase 7 — mechanical correction merge**: three audit fixes that read as clean string substitutions applied inline (AERO-002 `2035 → 2033` CNSA NSS deadline; AUTO-002 `superseded by → largely replaced in practice by` SAE J3061; AUS-FIN-002 redundant correction dropped — original prose already said "Reserve Bank Information and Transfer System"). **Phase 8 — LLM-grounded rewrite via qwen3.6:27b** (per `feedback-enrichment-default-model`): 54 rows with remaining appended Correction sentences fed through a two-stage pipeline — rewrite prompt (2–3 sentence tight prose incorporating every audit fact inline) followed by a strict self-grounding gate ("ACCEPT only if every audit correction is reflected; REJECT with reason otherwise"). 52 accepted (96%); 2 retained their visible Correction after both retries failed grounding (AUS-GOV-001 — Grover/Shor + AES-128/256 distinction; TELCO-005 — exact ETSI ISG QSC document count). **Phase 8.5 — hand-edit of the 2 grounding-rejected rows**: review confirmed both prose passages already incorporated the audit-corrected facts inline (AUS-GOV-001 distinguishes Grover weakening AES-128→64-bit from Shor's complete break of RSA/ECDSA and names AES-256 as the symmetric mitigation; TELCO-005 states "Study Item in Release 19, normative requirements expected in Release 20+" exactly as the audit demanded). Dropped the redundant appended Correction sentence on both rows; restored +10 on `accuracy_pct` + `confidence_score`. **Final outcome: 0 of 106 active rows carry a visible `Correction (audit …)` sentence — all 57 audit corrections reflected inline.**
- **Threats — `trusted_source_id` catalog schema fix + TP-3 validator (Phase 9, 2026-05-21)**: The `pqc_authoritative_sources_reference_*.csv` catalog now has a primary-key `id` column (kebab-case slug derived from `Source_Name`), so the 51 distinct `trusted_source_id` values that active threat rows reference can finally be programmatically joined to the catalog. New file `pqc_authoritative_sources_reference_05212026_r2.csv` (140 rows): `add-catalog-id-column-05212026.ts` derived deterministic slugs + applied 60 hand-overrides where the consuming CSVs already use shorter conventional ids (`nist-csrc`, `acsc`, `csa-cloud`, `iso-iec-sc27`, …); `backfill-catalog-missing-ids-05212026.ts` then added 32 missing catalog rows (AACS, APNIC, AWWA, BIS, CA/Browser Forum, CCSS, DVB Project, European Commission, EMVCo, EPA, ERA-Rail, FDA, FERC, Forescout, FPKI, FRA, Geneva Association, GRI, GS1, GSMA, ICAO, IEC, IMO, Internet2/InCommon, ITU, NYDFS, OpenSSF, OpenSSH, OpenSSL Project, PCI-SSC, US-DoE-Ed, WCO) so every threats reference resolves. **TP-3 validator** wired into `scripts/validators/threats-proof-rule.ts` and the blocking CI step in `.github/workflows/ci.yml`: every non-empty `trusted_source_id` on an active threat row must resolve to a catalog `id`. ERROR severity; smoke-tested against a simulated regression (renamed one row's `trusted_source_id` to `definitely-not-in-catalog` — TP-3 fired with a specific finding). The CI step's name updated to reflect TP-1 + TP-2 + TP-3 coverage. Catalog has not yet been backfilled for library/timeline/compliance/migrate references — TP-3 only enforces against the threats CSV today; extending to other 4 CSVs is a follow-up. Accepted rewrites get +10 restored on accuracy_pct and confidence_score with a traceability note in `data_quality_notes`. Wrapped in `caffeinate -i` per `feedback-caffeinate-long-runs`; ~20 min total runtime. **Outcome**: 106 active rows across all 23 industries (Cross-Industry 18, Financial 7, Gov/Defense 6, Telco 6, …, minimum Aerospace & HSM 2 each); only 6 truly hard-blocked rows remain deprecated after defeating curl + Playwright + Wayback + manual probing (AERO-001 FAA, AUTO-003 UNECE, ENERGY-002 NRC, AUS-GOV-002 cyber.gov.au, HLTH-001 HHS, IT-005 FIDO). RAG corpus regenerated (11,237 chunks, +24 reactivated threats), embeddings rebuilt (16.4 MB, sha256 advanced). `scripts/download-threats.js` regex extended to also match `_rN` revision suffixes so the downloader stays in sync with the loader. TP-1 + TP-2 gate continues to PASS with zero findings.
- **Threats — Phase 10: validator tests + audit reflection sanity-check + 4 more reactivations (2026-05-21)**: (1) **Unit tests for `threats-proof-rule.ts`** — new `scripts/validators/__tests__/threats-proof-rule.test.ts` ships 13 tests covering TP-1 happy path + 4 failure modes + deprecated-row ignore; TP-2 happy + outside-`public/threats/`; TP-3 happy + unresolved + migration-window-SKIP + empty-tsid; overall shape + no-CSV-skip. Required a small refactor: `DATA_DIR` resolved lazily via `() => path.join(process.cwd(), 'src/data')` so tests can chdir into per-worker tmp fixtures (production behaviour unchanged). All 13 pass. (2) **Audit-reflection sanity check** — new `scripts/verify-audit-corrections-reflected.py` extracts must-appear + must-not-appear tokens from each audit `correction` and cross-checks the current `threat_description`. Spot-checked the 15 heuristic-flagged "not reflected" rows manually — all are heuristic false positives (audit facts ARE in the prose; the regex stumbles on em-dashes, parenthetical column-name quotes, and Replace-from clauses with multi-word source tokens). (3) **One real bug surfaced** — `GOV-005` and `GOV-002` both still cited "2035" as the CNSA 2.0 full-transition deadline; per the AERO-002 audit fix that should be 2033 (CNSA 2.0 actual deadlines are 2030 preferred / 2033 required, never 2035). Applied the same `2035 → 2033` substitution to both rows and stamped `data_quality_notes`. Now zero active CNSA 2.0 rows mention 2035. (4) **4 more hard-blocked rows recovered** via Playwright at alternate primary sources: `AUTO-003` → EUR-Lex CELEX 42021X0387 (UN-ECE R155 published via EU OJ, 560 KB); `ENERGY-002` → ecfr.gov 10 CFR 73.54 (NRC cybersecurity rule, 81 KB); `AUS-GOV-002` → NLA web archive snapshot of the cyber.gov.au ISM cryptography page (7.7 KB); `HLTH-001` → ecfr.gov 45 CFR Part 164 Subpart C (HIPAA Security Rule, 150 KB). Active rows: **106 → 110**. (5) **3 new catalog rows backfilled** for the trusted_source_ids the reactivated rows reference (`unece`, `nrc`, `hhs`) so TP-3 stays clean. (6) **2 rows remain hard-blocked**: `AERO-001` (FAA — Federal Register URL variants 404; advisory-circular path also 404) and `IT-005` (FIDO — Playwright timeout after 60 s on both the FAQ and spec-overview URLs). Both stay `status='deprecated'` per the DS-series rule. All three gates (TP-1 / TP-2 / TP-3) PASS clean; RAG corpus regenerated (11,241 chunks); embeddings rebuilt.

...
- **Timeline — org-category filter (Government / Standards / Vendors)** [view:/timeline]: New 3-way category filter on the migration timeline ([CategoryFilter.tsx](src/components/Timeline/CategoryFilter.tsx)), backed by a new `entity_type` column on `timeline_*.csv` (`government` | `standards` | `vendor`). The default view shows **Government (153)** + **Standards & Consortia (29)**; **Vendors & Ecosystem (39)** — commercial products, quantum hardware, blockchain — is hidden by default and opt-in via the dropdown. URL-param state (`?cat=…`) makes filtered views shareable. Filtering is applied leaf-level in [TimelineView](src/components/Timeline/TimelineView.tsx)'s `ganttData` memo and composes with the existing region / country / trust-tier / search filters. `EntityType` type added to [timeline.ts](src/types/timeline.ts); the loader defaults missing values to `government`. All 107 distinct orgs classified into the three buckets (partnerships resolved by lead entity).
- **Library — multi-revision documents collapse into one tile** [view:/library]: Documents that existed as several records (older IETF drafts, obsoleted RFCs, prior standard editions) now render as a **single tile** showing the latest revision, instead of 3–4 near-duplicate tiles (e.g. Merkle-Tree-Certs showed `draft-ietf-tls-merkle-tree-certs` + `draft-ietf-plants-merkle-tree-certs-00/01` + `IETF-MTC-Draft-09` + the bare draft). New optional `superseded_by` column on `library_*.csv` links each deprecated revision to its surviving record. The loader ([libraryData.ts](src/data/libraryData.ts) → `attachPriorRevisions`) runs a second pass over the deprecated rows, groups them by `superseded_by`, and attaches them to the survivor as `priorRevisions[]` (deprecated rows are never emitted as their own tile). A new helper [`getGroupStatusBucket`](src/utils/documentStatusBucket.ts) computes the **most-advanced** lifecycle bucket across the group, so the surviving tile shows the furthest status reached (e.g. an AUTH48 revision) plus a muted "⎘ N earlier revisions" indicator ([DocumentCard.tsx](src/components/Library/DocumentCard.tsx)). The detail popover ([LibraryDetailPopover.tsx](src/components/Library/LibraryDetailPopover.tsx)) gains a **"Previous revisions"** section listing each older revision with a status chip + external source link. Same-RFC-number id-aliases (e.g. `IETF RFC 8391` collapsed under `RFC 8391`) are filtered out of `priorRevisions` so a pure duplicate isn't surfaced as a bogus "earlier revision" — the tile still dedupes, but the deprecated alias stays carried-forward + resolvable. Critical loader fix: the dependency tree is now built **last**, on the fully-enriched objects, so a record reached via `parent.children` (and via `findByRef` on a `?ref=` deep link) is the same object the card uses — previously the popover could resolve a stale child copy and show an empty "Previous revisions" list. Grouping is generated by `scripts/build-revision-groups.py` (canonical doc-key from the download URL + `deprecated_reason` backfill, homepage URLs excluded): **45 revision groups, 50 records consolidated, 0 false groups**.

### Data

- **Timeline — 100% authoritative-source coverage + native-language primaries** [view:/timeline]: Drove `source_url_quality` to **100% (234/234 active rows `url_authoritative`, 0 `url_needs_review`)** under a fix-or-drop policy. **16 of the last 23 flagged rows fixed** with authoritative primaries; **7 dropped/deprecated** (no fetchable primary after exhaustive anti-bot + Wayback + native search: China CACR, Israel INCD, Japan METI-NISC/NISC/NCO, Bahrain NCSC, Jordan CBJ). **8 fixes used non-English official sources fetched via the anti-bot resilient fetcher**, each cached with an **English translation of the relevant section** (`public/timeline/*.translation.md`): China NGCC ×2 (niccs.org.cn), China Telecom (chinatelecom.com.cn), South Korea MSIT ×2 (KISA seed.kisa.or.kr), Taiwan MODA ×2 (moda.gov.tw PQC Migration Guide), Japan NEDO K Program (prtimes.jp). Other fixes: Ethereum→ethereum.org, ADGM/TII→adgm.com, PQShield-VPN, Thales/Samsung→businesswire, UAE CDT→mediaoffice.abudhabi, UAE Encryption Policy→wam.ae, Spain CCN ×2→ccn-cert.cni.es. **India DST/NQM ×4 re-pointed** to the authoritative **May-2026 DST report "Quantum-Safe Ecosystem in India: Roadmap to Quantum Resiliency"** (was the Feb `(v1)` report), with descriptions clarified to the 3-milestone CII (2027/2028/2029) vs Enterprise (2028/2030/2033) tracks + 31-Dec hard deadlines + CBOM FY2027-28. Corpus orphan-clean (stripped 21 stale enrichment blocks; deprecated 8 stale concept-registry timeline slugs), regenerated (**11,414 chunks, 0 residual orphans**) and re-signed (kid 11b7, ML-DSA-65). 22 deprecated rows total filtered from the Gantt.
- **Timeline — gov-row primary follow-up + corpus orphan cleanup** [view:/timeline]: Web-researched authoritative primaries for the remaining flagged government rows; **3 more upgraded** (G7 CEG → treasury.gov "Statement on Planning for the Opportunities and Risks of Quantum Computing" PDF; Nigeria NDPC → ndpc.gov.ng official Act page; South Korea MSIT K-PQC → kpqc.cryptolab.co.kr consortium). **Authoritative coverage now 217/241 (90.0%)**; 23 rows remain flagged (no auto-reachable primary — China ×4, Japan gov ×4, Spain ×2, Taiwan ×2, UAE CSC ×2, Bahrain, Jordan, Israel; csc.gov.ae confirmed-exists but is an unfetchable JS SPA). Fixed 2 broken `local_file` pointers (NEDO, Vitalik) caught by a full Gantt milestone/phase audit — **all 241 active milestones/phases now resolve to an on-disk reference + an enrichment entry**. **Corpus orphan cleanup:** removed 14 stale-key orphan enrichment blocks, added a `getTimelineRefIds()` active-row filter in `processDocumentEnrichments` (the timeline collection lacked the deprecated-row guard the library collection had — latent leak), and deprecated 4 stale timeline slugs in the concept registry. Corpus 11,439 → **11,404 chunks, 0 residual orphans**, re-signed (kid 11b7).
- **Timeline — Gantt `PHASE_ORDER` completeness** [view:/timeline]: Added the `Deadline` phase to `PHASE_ORDER` in `SimpleGanttChart.tsx` (the 34 Deadline-phase rows previously got `indexOf === -1` and sorted to lane-front). All 22 SimpleGanttChart tests pass.
- **Timeline — secondary → primary URL upgrade pass** [view:/timeline]: Systematic promotion of `source_url_quality` from `url_needs_review` to `url_authoritative` by locating, fetching (resilient curl_cffi + playwright-stealth), keyword-verifying, and caching the **originating authoritative primary** for every probed row. **Authoritative coverage 180/240 (75%) → 214/241 (89.2%).** ~34 rows upgraded across waves: batch-5 (Czech NÚKIB ×2, PQCC, OASIS PKCS#11, UAE TII, CA/Browser Forum SMC013, Saudi NCA ECC-2); own-domain verification (Microsoft quantum-safe blog + 4 roadmap phases, PQShield ×3, SandboxAQ ×2, Quantinuum ×2, Google ×2, IDEMIA, Infineon, Adva, OpenSSL ×2, Algorand, ENISA, ISO/IEC SC27, 3GPP TR 33.938, Kenya ODPC); and secondary→primary swaps (EU ECCG → ENISA cert-portal PDF; Italy ACN → acn.gov.it; Germany BSI/DLR QUANTITY → qci.dlr.de; Bitcoin Core BIP-360 → github.com/bitcoin/bips; Japan NEC/NICT → nict.go.jp; SEALSQ → sealsq.com; BTQ → btq.com; Ethereum Foundation ×2 → blog.ethereum.org; African Union Malabo Convention → live au.int treaty page). **26 rows kept flagged `url_needs_review`** — real, cached-as-evidence events whose only reachable source is a news aggregator or a JS-rendered/non-English government SPA (China CACR/NGCC, G7 CEG, Spain CCN, Taiwan MODA, Korea MSIT, several Japan gov, UAE CSC, Bahrain, Jordan, Nigeria NDPC, Israel INCD); none deprecated (claims remain verifiable, just not from an auto-reachable primary). **34 upgraded rows re-enriched** from their freshly-cached primary docs (MLX `qwen3.6-27b-8bit`, batch=4). Manifest refreshed (+30 cached docs). Corpus regenerated (11,439 chunks) and re-signed (kid 11b7, ML-DSA-65).
- **Timeline — full reference audit: accuracy, freshness, 100% cache + scope classification** [view:/timeline]: Rebuilt the `public/timeline/` source cache from `timeline_06072026.csv` (was 0% on a fresh checkout — source docs are gitignored/machine-local) via `node scripts/download-timeline.js`. **Repaired 7 dead/wrong reference URLs** (SandboxAQ, FS-ISAC, Egypt MCIT, Kenya ODPC, UAE CSC, TCG ×2) and **corrected 3 factual date errors** surfaced by the dead-link triage: FS-ISAC PQC Timeline paper `2026-01 → 2025-09`, Egypt Law 151 executive regulations `2024 → 2025-11` (Ministerial Decree 816/2025), SandboxAQ `2025-12-01 → 2025-12-10` (and DoD → "Department of War"). **Dropped 1 unverifiable claim** (China NGCC financial-sector phase — no source documents it) and **1 duplicate** (generic TCG TPM row) per a drop-unverifiable policy. 14 bot-blocked government records (ASD ISM, BSI TR-02102, ITU-T X.1811, Nigeria NDPA, POPIA, Malabo Convention…) preserved via **Wayback snapshots** (originals kept in `data_quality_notes`). Result: **182 active / 14 deprecated, 100% (182/182) references cached, 0 failed.** New `entity_type` column classifies every row government/standards/vendor for the category filter; vendor/blockchain rows kept in-data (filtered, not deleted). Manifest refreshed.
- **Library — freshness refresh: 100 confirmed updates across 794 active records** [view:/library]: Multi-agent web sweep of every active library record against issuer/IETF sources, triple-verified (URL validation + successor-presence + adversarial web re-check). **72 records updated in-place** to their latest version (new doc downloaded + MLX `qwen3.6-27b` re-enriched), **21 obsoleted records deprecated** (successor RFC already tracked — TLS 1.2→1.3, IKEv2, DTLS→1.3, S/MIME, COSE, HTTP, CMP `RFC 4210→9810`), and **2 new RFC rows added** (`RFC-9345` ← `draft-ietf-tls-subcerts`; `RFC-9941` ← `draft-josefsson-ntruprime-ssh`). Notable version bumps: BSI TR-02102-1 (2024-01 → 2026-01), NIST SP 800-88 r2, SP 800-63-3 → 63-4, PKCS#11 v3.2, TCG TPM V185 final, EUDI Wallet ARF v2.9.0, ETSI/ANSSI editions. Recovered 18 of 27 portal/paywall-blocked docs via direct-URL lookup; 9 remain manual (ISO paywalled, PCI/MAS/CSA/CNSSP gated, cyber.gov.au/bikesuite bot-walled). New dated files `library_06052026.csv` → `library_06062026.csv`; `06042026` archived per the keep-2 rule. `superseded_by` column documented in `csv-status-schema.md`. Self-containment preserved (CM-SC/CM-SC-MD/CM-STATUS clean — 0 records dropped).
- **Library — source-doc download refresh + RAG `source_passages` restored** [view:/library]: Release-finalization pass closed a RAG provenance regression: the library refresh had changed/added referenceIds, but the TF-IDF `source-passages-*.json` (consumed by `generate-rag-corpus.ts` to populate `RAGChunkProvenance.source_passages[]`) was stale, so **690 library chunks shipped with a `source_doc` but no quoted passages** (vs 29 on main). Fixed by (1) running `download-library.js` to fetch the missing on-disk sources — **219 docs downloaded** (IETF datatracker drafts, NIST Round-2 signature specs on csrc, CISA/GSMA/ANSSI/BSI/NIST PDFs), `local_file`/`downloadable` updated on 139 rows → new `library_06072026.csv` (`06052026` archived, keep-2); and (2) re-running TF-IDF passage extraction over the now-697 local docs and regenerating the corpus — **chunks-without-passages 690 → 81** (under the 448 invariant; remainder are vendor JS-SPA pages + gated PDFs with no extractable text). Embeddings index rebuilt to match (`embeddings.bin` + `embeddings-meta.json`, corpusHash re-pinned) and `rag-corpus.json` re-signed (ML-DSA-65, kid `11b7`); all 8 trust artifacts verify. New corpus source `softhsmv3` (SoftHSM v3 dev-guide, 55 chunks) added to the tier-coverage exempt list alongside `openssl-guide`/`user-manual`.
- **Migrate — vendor_id referential-integrity fixes + catalog/roadmap/proof refresh** [view:/migrate]: Catalog advanced `pqc_product_catalog_06042026.csv` → `06062026_r4.csv` (**787 → 838 active products, +51**) and the vendor roadmap advanced `migrate_vendor_roadmap_05072026.csv` → `06062026.csv` (**50 → 115 vendor roadmaps, +65**). Headline fix for the **VND-ID collision data-mixup**: sequential catalog `vendor_id`s had silently collided with newly-added roadmap VND numbers, so products surfaced the wrong company's roadmap (e.g. **A10 Networks Thunder ADC displayed Trezor's roadmap** under `VND-355`). **27 vendor_id assignments corrected** — 18 cleared to empty (no roadmap link until a correct vendor exists: A10 Networks, Crypto Quantique, Kudelski, Ascertia ×3, Czertainly, EverTrust, MTG AG, Nexus, essendi, CrowdStrike, Hex Trust, Komainu, Descope, Stytch, Galileo, BeyondTrust, AnyDesk) and 9 remapped to the right vendor (`cryptonext-quantum-safe-library` → VND-011, `digicert-device-trust-manager` → VND-012, `dogtag-certificate-system` → VND-032, `f5-adsp-pqc` → VND-014, `ibm-cbomkit` → VND-019, `opensuse-leap` → VND-040, `splunk-enterprise` → VND-008). **9 roadmap vendors deprecated** (panel removed: Hex Trust, Komainu, Descope, Stytch, Galileo, BeyondTrust, CrowdStrike, Splunk/Cisco, AnyDesk) and **~11 roadmap source URLs upgraded** secondary → official primary (Apple, F5, Fortinet, Futurex, NXP, Red Hat, Thales, Forward Edge-AI, Adtran, Orange, Nord Security). **Zscaler (VND-190) reasoning-leak cleaned** — the Hybrid Mode Support field was leaking Qwen3 chain-of-thought into the UI; re-enriched to a clean "Yes; hybrid ECC+ML-KEM key-exchange strategy". New `catalog_doc_enrichments_06062026.md` (868 products, MLX `qwen3.6-27b-8bit`) and `migrate_certification_xref_06062026.csv` (1188 product↔cert links) drive richer expanded rows. Proof manifest (`public/migrate-proofs/manifest.json`, 785 downloaded sources / 591 ok) backs the catalog refresh; proof HTML/PDF binaries stay local-only (gitignored, 228 MB).
- **Threats — recovered 7 bot-blocked evidence sources + batch re-enrichment** [view:/threats]: The 7 industry-threat rows whose evidence downloads had failed under plain Node fetch (`AUTO-001/003/004` UNECE UN-R155, `TELCO-001` GSMA PQC, `TELCO-004` GSMA eSIM, `EDU-004` DoD CMMC, `WATER-005` FERC hydro) were recovered via the resilient fetcher (curl_cffi browser-TLS impersonation → playwright-stealth fallback) — **`public/threats/evidence/manifest.json` now 112/112 ok (was 105/112)**. All 7 re-enriched from their freshly-cached primary docs via the optimized batch MLX path (`batch_generate`, batch=4, `qwen3.6-27b-8bit`), matching the timeline/product pipelines; empty-field counts dropped on 3 of 4 sampled rows (`AUTO-001` 33→26, `TELCO-001` 35→21, `EDU-004` 31→23; `WATER-005` flat — the FERC page genuinely carries no PQC content). New `threats_doc_enrichments_06072026.md` (194 records, self-contained via DS05p2 carry-forward; loader merges newest-wins).
- **Patents — refresh to 928 + verification-column backfill** [view:/patents]: Advanced the PQC patents corpus `patents_05032026.csv` → `patents_06072026.csv` (**904 → 928, +24**). The 24 new grants (published since 2026-05-01, deduped against the existing corpus) were fully enriched across all 32 dimensions — incl. `quantum_relevance` + `pqc_migration_score` — via local MLX `qwen3.6-27b-8bit`; **3 image-only recent grants recovered via OCR**. Source is a USPTO/patent-DB search export (now gitignored as `SearchResults*.csv` scratch input). Follow-up **verification pass** date-stamped 2026-06-07 backfills the 4 verification columns (`citation_graph_flag`, `pqc_migration_score_flag`, `dates_verified_iso8601`, `date_stamp`) that the +24 add had left empty pending verification — the corpus now carries `citations_present:<n>` citation-graph flags, `high/low_pqc_relevance` score flags, and ISO-8601 verified dates across all 928 rows. A consolidation pass recovered **197 previously-incomplete rows** whose enrichment existed only as per-patent markdown and was never wired into the hub CSV — the corpus now carries **0 stub rows** and 100% `quantum_relevance` coverage, and the normalize step preserves prior `pqc_migration_score` on re-enrichment so the upgrade adds records without dropping any relevance badges. `patents_05032026.csv` archived per the keep-2 rule.
...
- **Timeline — deprecated rows excluded from the Gantt; graded confidence; schema cleanup** [view:/timeline]: [timelineData.ts](src/data/timelineData.ts) now calls `filterActive`, so DS01 `status=deprecated` rows are excluded from the Gantt and the New/Updated diff (the lowercase `status` column was previously parsed away entirely — now matching the other DS01 loaders). The binary `confidence_score` (only `60`/`85`) is replaced by a transparent graded **0–100** `computeTimelineConfidence()` computed at load time from source-URL quality, peer-review + vetting body, source recency, cached evidence, and date specificity. A recently-appended industry batch using out-of-schema `Category`/`Type` values (Blockchain, Hardware, Network Security, Publication, Federal Mandate…) is remapped to valid phases, 4 invalid `Type`s normalized, and malformed-Country duplicate rows (org/flag codes in the Country column) deprecated. The duplicate CISA "Product Category List" row is split into the accurate publication (Jan 2026) + EO-14306 deadline (Dec 2025) events. Enrichment made **durably cumulative** — a new `DS05p3` CSV-membership filter in the carry-forward (`enrich-docs-ollama.py`, private repo) stops superseded/renamed records from resurrecting into each new cumulative `timeline_doc_enrichments_*.md`; 19 stale orphan entries pruned.
...
- **Migrate — vendor-ref referential-integrity audit gate** [view:CI]: New `scripts/audit-vendor-refs.ts` (+ `npm run audit:vendor-refs`, wired into `.github/workflows/ci.yml` after `audit:matrix-refs`) guards the product catalog's `vendor_id` column against the exact class of bug that surfaced Trezor's roadmap on A10 Networks. For every active product whose `vendor_id` **resolves in the vendor roadmap CSV**, it fails the build on (A) a reference to a `deprecated` roadmap vendor and warns on (B) a name-token mismatch between the `product_id` / `vendor_name_original` and the resolved roadmap vendor name. Products whose VND has no roadmap entry yet are intentionally skipped (expected, not an error). A self-documenting `KNOWN_INDIRECT` allowlist (38 entries) carries the legitimate indirect mappings where a product's trading name differs from its `product_id` (`aws-*`→VND-001, `nginx`→VND-014, `ubuntu-`→VND-006, `whatsapp`→VND-118, …). Gate is read-only against public data; current catalog passes clean.

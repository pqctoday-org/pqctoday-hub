...
- **Timeline — secondary → primary URL upgrade pass** [view:/timeline]: Probed official government/standards-body URLs for all 10 remaining `url_needs_review` non-vendor rows (ECCG, NEDO, Japan NCO, UAE CSC Encryption Policy, UAE CDT, Jordan CBJ, Egypt MCIT, China NGCC ×2, South Korea K-PQC). **1 upgrade applied:** EU ECCG "Agreed Cryptographic Algorithms v2" source upgraded from pqshield.com (secondary aggregation page) to the **official ENISA certification-portal PDF** (`certification.enisa.europa.eu/.../EUCC_guidelines_Agreed Cryptographic Mechanisms v2.pdf`, 346 KB); `source_url_quality` promoted to `url_authoritative`. **9 remaining secondaries confirmed live** and appropriate as best-available sources — primary pages are either JavaScript-rendered SPAs not accessible to resilient fetch, in non-English only, or exist only at homepage depth (no stable article URL). Corpus regenerated (11,433 chunks) and re-signed (kid 11b7).

...
- **The executive exposure card drew the wrong conclusion from its own numbers** [view:/] [persona:executive]: it listed twelve years of data secrecy and a five-year migration, then concluded you were four years short. The correct answer from those figures is eleven. The date a quantum computer is expected to break today's cryptography was also out of step with the figure the Threats page uses. Both the conclusion and the date are now calculated from the site's own single source, so they cannot drift from the numbers printed above them.

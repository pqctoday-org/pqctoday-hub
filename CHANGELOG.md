...
- **Protocol Support spec links open the in-app Library entry** [view:/algorithms] [persona:architect] [persona:developer]: clicking an RFC or draft in a protocol's Specifications (or a matrix cell) now opens that document's Library reference inside the app instead of bouncing you to the raw external RFC page. Specs not yet held in the Library still link out, now clearly marked as external.

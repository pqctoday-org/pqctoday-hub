...
- **Crypto-Agility Control Plane (CACP) Playground** (`/playground/cacp`): a full KMIP 3.0 crypto-agility control plane running entirely in the browser — the `pqctoday-kmip` policy engine + `softhsmrustv3` PKCS#11 v3.2 engine compiled to WebAssembly (v0.17.0), with no server. Real ML-KEM / ML-DSA / SLH-DSA / RSA / ECDSA / Ed25519 / AES crypto plus classical X25519/X448 key agreement, driven by loadable YAML policies.
  - **Agility & Workbench tab**: pick a policy, then create keys and run Sign/Verify, Encrypt/Decrypt, and Encapsulate/Decapsulate — watch the SAME request get allowed, denied, or auto-rekeyed as you flip the policy; every call shows the TTLV wire response and the cross-plane (policy / KMIP / PKCS#11) audit trail.
  - **Policy tab**: browse and load the bundled crypto-agility policies (classical, PQC, hybrid, CNSA 2.0, BSI TR-02102, FIPS-only, migration windows, and the label-only migration policies); visual, matrix, rules, timeline, and raw-YAML views, plus validated positive/negative test scenarios.
  - **KMIP 3.0 tab**: a **Learn** sub-tab with guided classical→PQC walkthroughs that run for real against the engine, a **Reference** sub-tab (all 66 KMIP 3.0 operations with parameter forms and a shared glossary), a **Command Lab** that runs certificate operations — Certify, Validate, and public-key verification — for real in the browser with per-operation forms and knowledge checks, a **Dev** pipeline builder whose palette switches between standard operation primitives and a live replay of the OASIS KMIP 3.0 conformance corpus, and a **Batch & Macros** runner.
  - **Migration tab**: validates a complete PQC migration on a seven-key business estate. Keys are generated by _business label only_ — the active policy decides every algorithm — then switching **Classical → Hybrid → Full PQC** rekeys each vulnerable key to its post-quantum successor on first use (AES-128→AES-256, RSA-2048/ECDSA-P256/Ed25519→ML-DSA-44, X25519/X448→ML-KEM or X25519MLKEM768). Includes a migration map (label → operation → algorithm per mode), a live key-object inspector with old→new rekey lineage, and a per-key collapsible KMIP log tagged by mode.
- **OpenSSL Studio**: Browser-based OpenSSL v3.6.3 workbench powered by WebAssembly
...
- **Learning Platform**: Comprehensive educational modules for cryptography and security
  ...
  - **Digital Assets Program** (Learn | Workshop | Exercises): Blockchain cryptography deep-dive
    - **Architecture Overview**: Blockchain cryptographic requirements table (Bitcoin secp256k1/ECDSA, Ethereum Keccak-256, Solana Ed25519) with OpenSSL 3.6.3 support matrix per operation and chain
    ...
    - **PQC Defense** (5-part): Quantum threat landscape, Bitcoin BIP-360 (P2QRH), Ethereum AA (EIP-4337/7702), Solana structural challenge, and **live ML-DSA-65 vs secp256k1 side-by-side demo** with real OpenSSL WASM v3.6.3 crypto — key gen, sign, verify, and BIP-360 witness overhead calculation
...
- **Simulation / "Mission Control"** (`/simulation`, full-viewport console, no nav
  chrome): a serious-game layer over the whole hub — pick an organisation profile,
  race the Mosca clock (X+Y>Z) against a configurable Q-Day planning anchor, and
  climb a 9-phase (P0–P7 plus a terminal Verification & Closure band) 0–4 maturity
  ladder by staffing/briefing an AI team and choosing sound-vs-trap "next moves."
  Every resource surfaced inside a phase is a real Command Center tool, Playground
  sandbox, or Learn workshop embedded live, not a mocked preview; state persists via
  `useSimulationStore`.
- **Explore** (`/explore`): a persona-aware site map — tiles for every major surface,
  reordered so the current persona's recommended paths surface first; gated for the
  Curious persona until onboarding unlocks the full set.
- **Navigate** (`/navigate`): full-page force-directed cluster graph of PQC concepts
  and modules (the standalone counterpart to the Right Panel's Knowledge Graph
  "Explore" tab below), with clickable category/sub-category/node labels.
- **Patents** (`/patents`): searchable table of PQC-relevant patent filings with a
  filing-year chart and column picker.
- **Revision History** (`/revisions`): a public, chronological feed of real content
  edits — module/tool changes, data-source updates — read from
  `public/data/revisions.jsonl`, with a rolling 30-day activity summary. See
  [CONTENT-FRESHNESS.md](CONTENT-FRESHNESS.md) for how entries get there.
- **FAQ**, **Sponsor**, and **Editorial Independence** pages (`/faq`, `/sponsor`,
  `/editorial-independence`): supporting/trust pages linked from the site footer and
  About page.

> 📋 The feature list above is generated by reading the routes under `src/App.tsx`
> and their components directly — there is no separate `REQUIREMENTS.md` spec file
> in this repository.

## Tech Stack

- **Frontend**: React 19 + TypeScript + Vite 7 (client-side SPA via `react-router`; no server-side rendering framework — pages are pre-rendered as static HTML at build time for SEO, see `npm run prerender`)
- **Cryptography**:
  - OpenSSL WASM v3.6.3 (with native ML-KEM, ML-DSA, and LMS/HSS support)
  ...
  - `pqctoday-kmip` / `softhsmrustv3` (v0.17.0) — the in-browser KMIP 3.0 crypto-agility control plane behind `/playground/cacp`: policy engine + PKCS#11 v3.2 Rust engine compiled to WASM; ML-KEM / ML-DSA / SLH-DSA / RSA / ECDSA / Ed25519 / AES, classical X25519/X448 key agreement, hybrid X25519MLKEM768, pure-Rust certificate operations (Certify/Validate/SPKI verify), label-only policy-driven migration with rekey-on-use
...
- Node.js **v22.22.0 or higher** (pinned in `.node-version` / `package.json#engines`;
  `fnm use` or `nvm use` in the repo root picks this up automatically)
- npm
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for the full picture — including how to
contribute **without writing code** (data-correction and case-study issue
templates; most contributions to this project are knowledge, not code) and
details on the CACP/KMIP and PKCS#11 test gates. Quick version for a code PR:
...
> The scraping/enrichment/discovery tooling itself lives in a private companion
> repository (not part of this public repo) and is triggered manually — there is no
> cron job or scheduled workflow anywhere in this project. See
> [CONTENT-FRESHNESS.md](CONTENT-FRESHNESS.md) for what's tracked automatically
> in-repo (freshness audits, module review dates, the revision log) versus what's
> hand-curated upstream.

## Architecture Overview

The application is structured into several key components:

- **`scripts/`**: ~90 standalone TypeScript/Python/JS scripts run via `tsx`/`npm run
audit:*` — data-integrity validators, CI audit gates (the `audit-*.ts` family, one
  script per named CI/pre-push check), generators (`generate-*.ts`, sitemap/OSCAL/CBOM/
  embeddings), and a handful of Python enrichment scripts. Most are wired into
  `package.json` scripts and either CI (`.github/workflows/ci.yml`) or the
  `.husky/pre-push` gate — see [TESTING.md](TESTING.md).
...
Dependency vulnerabilities are enforced continuously, not audited on a fixed
date: `npm run audit:deps` runs on every PR and push to `main`
(`scripts/ci/audit-gate.ts`) and fails the build on any high/critical advisory
in production dependencies that isn't covered by a short, explicitly dated
exception in that script — an exception must justify itself and expires
automatically. See [SECURITY.md](SECURITY.md) for how that gate works, how
client-side secrets (API keys, OAuth tokens) are and aren't handled, and how
to report a vulnerability. For the current point-in-time numbers, run
`npm audit` yourself or check the live SBOM/CVE summary on the
[About page](/about).

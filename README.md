...
- **Crypto-Agility Control Plane (CACP) Playground** (`/playground/cacp`): a full KMIP 3.0 crypto-agility control plane running entirely in the browser — the `pqctoday-kmip` policy engine + `softhsmrustv3` PKCS#11 v3.2 engine compiled to WebAssembly (v0.10.0), with no server. Real ML-KEM / ML-DSA / SLH-DSA / RSA / ECDSA / Ed25519 / AES crypto plus classical X25519/X448 key agreement, driven by loadable YAML policies.
  ...
  - **KMIP 3.0 tab**: a **Learn** sub-tab with guided classical→PQC walkthroughs that run for real against the engine, a **Reference** sub-tab (all 66 KMIP 3.0 operations with parameter forms and a shared glossary), a live **Corpus Replay** of the OASIS KMIP 3.0 conformance corpus, and a **Batch & Macros** runner.
...
- **Cryptography**:
  ...
  - `pqctoday-kmip` / `softhsmrustv3` (v0.10.0) — the in-browser KMIP 3.0 crypto-agility control plane behind `/playground/cacp`: policy engine + PKCS#11 v3.2 Rust engine compiled to WASM; ML-KEM / ML-DSA / SLH-DSA / RSA / ECDSA / Ed25519 / AES, classical X25519/X448 key agreement, hybrid X25519MLKEM768, label-only policy-driven migration with rekey-on-use

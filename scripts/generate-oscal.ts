...
// (e.g. `compliance_06042026.csv` → `2026-06-04T00:00:00.000Z`).
// Drives `last-modified` + all `start` fields so identical input data always
// produces a byte-identical OSCAL document — required for the trust-engine
// signature chain to stay valid across rebuilds without a wall-clock-driven
// re-sign.
...
async function main() {
  ...
  const dataVersion = dataVersionFromCsvName(latest)

  // 5. Build OSCAL result objects
  const governanceResults = governanceRows.map((r) => buildResult(r, reviewerMap, dataVersion))
  const fullInventoryResults = rows.map((r) => buildResult(r, reviewerMap, dataVersion))

...
// Optional --input-csv <path> override: skip auto-discovery and read from the
// given CSV. Used by Phase-2 download pass for the dataset 05062026 promotion
// to consume tasks/patch/proposed/timeline_proposed_*.csv.
const inputCsvIdx = process.argv.indexOf('--input-csv')
const INPUT_CSV = inputCsvIdx >= 0 ? process.argv[inputCsvIdx + 1] : null
...
async function main() {
  ...
  // Load CSV
  const csvPath = INPUT_CSV ? INPUT_CSV : findLatestTimelineCSV()
  console.log(`Source CSV: ${csvPath.replace(ROOT + '/', '')}\n`)
  if (INPUT_CSV) console.log(`(Override via --input-csv)\n`)
  ...
  for (const row of uniqueRows) {
    ...
    // Full title — used as the manifest label and downstream as the
    // enrichment ref_id. Previously sliced at 50 chars, which truncated
    // long titles (e.g. "ICP-Brasil Mandates ML-DSA and ML-KEM Certificate
    // Standards" lost the trailing "Standards") and orphaned the resulting
    // doc-enrichment chunks against the trust-score map.
    const label = `${country}:${orgName} — ${title}`

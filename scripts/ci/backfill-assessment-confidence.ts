...
/**
 * backfill-assessment-confidence.ts
 *
 * Adds confidence_score to the assessment CSV based on migration_priority field.
 *
 * Migration priority → confidence:
 *   critical  →  85
 *   high      →  70
 *   medium    →  60
 *   low       →  30
 *   default   →  30
 *
 * Usage: npx tsx scripts/ci/backfill-assessment-confidence.ts [--dry-run]
 */
import fs from 'fs'
...
function computeScore(row: Record<string, string>): number {
  const priority = (row['migration_priority'] ?? '').toLowerCase().trim()
  if (priority === 'critical') return 85
...
async function main() {
  const files = await glob('pqcassessment_*.csv', { cwd: DATA_DIR })
  files.sort()
  const latest = files.at(-1)
  if (!latest) { console.error('No assessment CSV found'); process.exit(1) }
  ...
  if (DRY_RUN) {
    console.log(`[dry-run] Would write ${rows.length} rows → ${outPath}`)
  } else {

...
/**
 * backfill-algo-confidence.ts
 *
 * Reads the latest algorithm reference CSV and adds a confidence_score column.
 *
 * Confidence rules:
 *   peer_reviewed=yes  AND  vetting_body non-empty  →  85
...
async function main() {
  ...
  if (!latest) { console.error('No algorithm CSV found'); process.exit(1) }

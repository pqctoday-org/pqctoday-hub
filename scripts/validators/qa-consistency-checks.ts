...
/**
 * Known regulatory and standards framework names that Q&A `compliance_refs`
 * may legitimately cite at the framework level instead of pointing at a
 * specific compliance-data.json record (which is keyed by FIPS/CC certificate
 * ID). These pass QA-C6 even when the compliance CSV doesn't have an exact
 * record matching the framework name.
 */
const KNOWN_COMPLIANCE_FRAMEWORKS = new Set<string>([
  // Regulators / agencies (referenced as authoritative bodies)
  'NIST',
  'NSA',
  'BSI',
  'ANSSI',
  ...
  'KISA',
  'ACVP',
  // Cross-sector regulatory frameworks
  'GDPR',
  'DORA',
  'NIS2',
  'eIDAS',
  ...
  'UNECE R155',
  'ISO 26262',
  // Generic certification frameworks (not a specific cert)
  'Common Criteria',
  'EUCC',
  'FIPS 140',
])
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    qaRows.forEach((row, i) => {
      for (const ref of splitSemicolon(row.compliance_refs)) {
        if (!complianceIds.has(ref) && !KNOWN_COMPLIANCE_FRAMEWORKS.has(ref))
          f.push(
            finding(
              qaFile.file,
              i + 2,

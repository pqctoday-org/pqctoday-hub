...
| `compliance-focus-view.spec.ts` (2)                                                                                                    | **RESOLVED 2026-07-16** — spec deleted | **STALE-REMOVED** — the "Focus view" master-detail UI was deleted outright when `/compliance` was rebuilt around a 3-pillar pipeline (`PillarPipeline.tsx` replaced `LandscapeTab.tsx`). No load issue; the button doesn't exist. The now-dead source files (`LandscapeTab.tsx`, `FrameworkFocusView.tsx`, and 7 components only they imported) were deleted the same day (compliance-maintenance audit, Phase 4.3) — the "restore vs delete" decision item 6 below asked for is now moot.      |
...
6. **`compliance-focus-view`** — RESOLVED 2026-07-16, spec + dead source deleted (see table row above). **`library-cswp39`** still needs the same product decision (restore the removed UI, or delete the tests for a feature that's genuinely gone) rather than a mechanical fix. Flag for whoever owns that page rather than fixing blind.

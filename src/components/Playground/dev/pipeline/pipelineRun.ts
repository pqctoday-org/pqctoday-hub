 ...
 * this header comment — full hub→sandbox decoupling stands (no clickable
 * links, no embeds), so the handoff is prose naming the channel, not a URL.
 * `lane` is a short human label ("PKCS#11 v3.2", "KMIP 3.0 + CACP").
 */
export function pipelineProvenanceHeader(lane: string): string {
  const version = typeof __APP_VERSION__ !== 'undefined' ? __APP_VERSION__ : 'dev'
  const date = new Date().toISOString().slice(0, 10)
  return [
    `# Generated by the PQC Today hub's ${lane} Developer tab (v${version}, ${date}).`,
    '#',
    '# This script ran in-browser, entirely client-side, against a WASM build of the',
    '# same engine the dev sandbox uses — no server, no network hop for the crypto',
    "# itself. It is written to also run UNMODIFIED in pqctoday's separately",
    '# distributed dev sandbox container (see the pqctoday-sandbox project — GHCR',
    '# packages and the repo README carry the current pull instructions), where the',
    '# same import resolves to the real native library instead of this in-browser shim.',
    '',
  ].join('\n')
}

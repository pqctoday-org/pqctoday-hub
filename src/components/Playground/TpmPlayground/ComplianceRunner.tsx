...
export function ComplianceRunner() {
  ...
  const runSuite = async () => {
    ...
    try {
      // The whole suite — every phase, every check — must run as one
      // exclusive operation against the shared WASM TPM instance, or
      // another panel's commands can land between two checks and corrupt
      // shared transient-object state (see tpmBridge.ts's withTpmLock and
      // the 2026-07-24 TPM playground concurrency remediation).
      await withTpmLock(async () => {
        ...
        addLine(
          'send',
          '           template      = restricted KEM EK  [TCG V1.85 §11.2.6 Table 204]'
        )
        addLine('send', '         )')
          ...
          if (akHandle === 0) {
            ...
            if (h.rc !== 0) {
              ...
              if (sigAlg === ALG_MLDSA && sigSize === MLDSA_65_SIG_SIZE) {
                ...
                addLine(
                  'recv',
                  `    ← signature = ${sigSize} B (FIPS 204 ML-DSA-65: 3309 B) ✓`,
                  true
                )
        ...
        addLine(
          'send',
          '    → TPM2_VerifySequenceComplete(@seq, keyHandle, signature)  [Table 118]'
        )

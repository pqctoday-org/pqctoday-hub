...
export type PlaygroundViewMode = 'path' | 'stack' | 'cards' | 'table' | 'nice'

interface PlaygroundViewToggleProps {
  mode: PlaygroundViewMode
  onChange: (mode: PlaygroundViewMode) => void
  /** Hide the 'path' option when no persona is active (path mode requires a persona). */
  pathAvailable?: boolean
}

const OPTIONS: { value: PlaygroundViewMode; label: string; icon: typeof Layers }[] = [
...
export const PlaygroundViewToggle = ({
  mode,
  onChange,
  pathAvailable = true,
}: PlaygroundViewToggleProps) => {

  ...
  try {
    if (p.dryKind === 'algo' && p.ops) {
      ...
      for (const op of p.ops)
        worst = moreRestrictive(worst, engine.dryRun({ op, algorithm: p.algorithm, length: p.length }))
      d = worst ?? { kind: 'Allow' }
    } else {
...
function liveRow(p: Probe, res: BatchResult): Row {
  ...
  let kind: Verdict = d.kind === 'Unknown' ? (created?.ok ? 'Allow' : 'Deny') : d.kind
...
export function PolicyScenario({
  ...
  // never shows a result from a policy you're no longer on.
  useEffect(() => {
    setRows([])
    setMode(null)
  }, [policyFingerprint])
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1 max-w-xl">
            ...
            <span className="font-semibold text-foreground">Run for real</span> executes them —
            keys appear in the Keystore and every step is logged in the Activity trail below. Your
            policy selection never changes either way.
          </p>
        </div>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`rounded px-1.5 text-[10px] font-semibold ${TONE[kind].badge}`}>
                      {kind}
                    </span>

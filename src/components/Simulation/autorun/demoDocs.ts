...
const crqcScenario: Record<DemoSector, DemoDoc> = {
  ...
  government: {
    ...
    data: md(
      ...
      '',
      '## Mosca inequality',
      "- X (secrecy) ≈ 25y, Y (migration) ≈ 5y, Z (CRQC) ≈ 5–9y → X + Y > Z. The June 2026 EO's 2030 key-establishment deadline is binding (NSM-10 set the 2035 whole-of-government goal; CNSA 2.0 sets the 2035 NSS completion).",
      MOSCA_PLANNING_NOTE
    ),
  },
  energy: {
  ...
  retail: {
    ...
    data: md(
      ...
      '',
      '## Mosca inequality',
      '- X (secrecy) ≈ 7y, Y (migration) ≈ 4y, Z (CRQC) ≈ 5–9y → X + Y > Z. PCI SSC added PQC requirements to PTS HSM v5.0 (May 2026), but PCI DSS itself has published no PQC requirements or v5.0 timeline yet.',
      MOSCA_PLANNING_NOTE
    ),
  },
  general: {

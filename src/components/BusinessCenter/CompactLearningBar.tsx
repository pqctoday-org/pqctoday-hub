...
import { GraduationCap, ExternalLink, Bookmark, ChevronDown } from 'lucide-react'
...
interface CompactLearningBarProps {
  ...
  /** Renders behind a <details> toggle, collapsed by default, so a first-time
   *  visitor at advanced density reaches the zone panels above faster. The
   *  header (icon, title, module count) stays visible in the <summary> either
   *  way — only the progress bar / CTA / bookmarks collapse. */
  defaultCollapsed?: boolean
}

export function CompactLearningBar({ modules, defaultCollapsed = false }: CompactLearningBarProps) {
  ...
  const bookmarksRow = myLearnModules.length > 0 && (
  ...
  if (defaultCollapsed) {
    // Collapsed by default (advanced density) — header stays visible in the
    // <summary> so the section is legible even closed; only the bookmarks
    // row, progress bar, and CTA are behind the toggle (no duplicated text).
    return (
      <details className="glass-panel group [&[open]>summary>svg.chev]:rotate-180">
        <summary className="p-4 cursor-pointer list-none select-none flex items-center gap-3">
          {header}
          <ChevronDown
            size={16}
            className="chev text-muted-foreground shrink-0 transition-transform"
            aria-hidden
          />
        </summary>

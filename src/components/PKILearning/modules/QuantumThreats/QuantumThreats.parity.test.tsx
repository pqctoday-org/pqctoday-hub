...
/**
 * Render-parity golden test for the QuantumThreats ModuleShell conversion.
 * Captured against the PRE-conversion module; must stay green after
 * QuantumThreats adopts <ModuleShell>, proving the conversion is
...
import { QuantumThreatsModule } from './index'

describe('QuantumThreats render parity', () => {
  it('renders the gradient header, the in-page description, and all six tabs', () => {
    render(
      <EmbedProvider>
        <MemoryRouter>
          <QuantumThreatsModule />
        </MemoryRouter>
      </EmbedProvider>
    )
    // header title
    expect(screen.getByRole('heading', { name: 'Quantum Threat Mechanics' })).toBeInTheDocument()

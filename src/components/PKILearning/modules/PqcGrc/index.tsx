...
import { Trash2, BarChart3, GitMerge } from 'lucide-react'
...
export const PqcGrcModule: React.FC = () => {
  ...
  const handleReset = () => {
    if (confirm('Restart PQC GRC Module?')) {
      setCurrentPart(0)
      startTimeRef.current = Date.now()
      updateModuleProgress(MODULE_ID, {
        status: 'in-progress',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gradient">PQC GRC</h1>
          <p className="text-muted-foreground mt-2">
            Make the post-quantum migration governable, measurable, and auditable: the KRI cascade,
            risk appetite, regulatory horizon, and the GRC-SOC handoff.
          </p>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="workshop">
          <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="glass-panel p-4 sm:p-6 md:p-8 min-h-[400px] md:min-h-[600px] animate-fade-in">
              ...
              />
              {currentPart === 0 && <KriCascadeBuilder />}
              {currentPart === 1 && <ExceptionRegisterTriage />}
            </div>
        ...
        <TabsContent value="exercises">
          <PqcGrcExercises onNavigateToWorkshop={handleExerciseNavigate} />
        </TabsContent>

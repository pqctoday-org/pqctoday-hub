...
/**
 * Render-parity golden test for the PlatformEngPQC ModuleShell conversion.
 * Captured against the PRE-conversion module; must stay green after
 * PlatformEngPQC adopts <ModuleShell>, proving the conversion is
...
import { PlatformEngPQCModule } from './index'

describe('PlatformEngPQC render parity', () => {
  it('renders the gradient header, the in-page description, and all six tabs', () => {
    render(
      <EmbedProvider>
        <MemoryRouter>
          <PlatformEngPQCModule />
        </MemoryRouter>
      </EmbedProvider>
    )
    // header title (renders the literal "&" — JSX entity in source only)
    expect(screen.getByRole('heading', { name: 'Platform Engineering & PQC' })).toBeInTheDocument()

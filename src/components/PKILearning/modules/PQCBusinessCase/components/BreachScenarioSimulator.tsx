    ...
    <div className="space-y-8">
      ...
      <details className="glass-panel p-4">
        ...
        <div className="mt-3 space-y-2 text-xs text-muted-foreground leading-relaxed">
          ...
          <p>
            <strong className="text-foreground/80">2. How much of the harvested corpus still
            matters.</strong> Freshness is integrated across data aged 0 to {breachCosts.yearsOfData} years
            against the {DATA_SENSITIVITY_LABELS[breachCosts.dataSensitivityClass].toLowerCase()} shelf life of{' '}
            {DATA_SHELF_LIFE_YEARS[breachCosts.dataSensitivityClass]} years. Old data contributes less; a corpus
            older than its shelf life saturates rather than dropping to zero, because its freshest
            layer always still counts.
          </p>
          <p>
          ...
          <p>
            <strong className="text-foreground/80">4. Weighting by whether a CRQC exists at
            all.</strong> Steps 1–3 give the cost <em>if</em> a machine capable of breaking today&apos;s
            keys exists. The expected annual loss blends that against the no-CRQC case using the
            GRI 2025 survey&apos;s arrival curve — currently a{' '}
            {Math.round(breachCosts.pCrqc * 100)}% chance within your planning horizon.
          </p>
          <p>
          ...
          <p className="pt-1 border-t border-border/50">
            Shelf-life figures are labelled assumptions rather than cited standards for four of the
            five data classes. The arrival curve is one expert survey, not a forecast. Treat the
            output as a structured argument, not a measurement.
          </p>
        </div>
      </details>
      )}

...
/**
 * The SOC use cases moved to `@/data/socQuantumPlaybook` (2026-09-23) so the
 * Threats page could read the same list. The Learn module must render exactly
 * what it rendered before the move — these are the literal values it carried
 * inline until then.
 */
...
describe('SocImplementationPqc use cases are unchanged by the move to the shared module', () => {
  it('Detection Planner rows: same ids, codes, titles, summaries and severities', () => {
    expect(DETECTION_USE_CASES).toEqual([
      {
        id: 'hybrid-downgrade',
        code: 'UC-1',
  ...
  it('Why It Matters cards: same titles', () => {
    expect(SOC_GUIDE_DATA.threatImpacts.map((t) => [t.id, t.title])).toEqual([
      ['hybrid-downgrade', 'Use Case 1 — Hybrid Downgrade Detection'],
      ['crypto-drift', 'Use Case 2 — Cryptographic Drift Monitoring'],
      ['cert-lifecycle-anomalies', 'Use Case 3 — Certificate Lifecycle Anomalies'],
      ['tnfl-signature-integrity', 'Use Case 4 — TNFL & Signature Integrity Monitoring'],
      ['hndl-indicator', 'Use Case 5 — Enhanced HNDL-Indicator Detection'],
    ])
  })
})

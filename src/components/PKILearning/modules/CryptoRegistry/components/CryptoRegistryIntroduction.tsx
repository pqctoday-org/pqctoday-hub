    ...
    <div className="space-y-5">
      ...
      <Section id="registry-why" icon={BookOpen} title="Why a registry, not just a spec">
        <p>
          ...
          certificate, <code>ES256</code> to a JWT, code point <code>0x0403</code> to TLS 1.3. A
          CBOM (or any inventory) built from several discovery tools inherits all of these
          inconsistent names — and nothing lines up until you normalize them.
        </p>
        <p>
          <Badge kind="std" />
          CycloneDX v1.7 answers this with the <strong>Cryptography Registry</strong>: one
          versioned, machine-readable list of canonical algorithm-family and elliptic-curve names,
          each backed by its standardization reference (RFC, FIPS, IEEE, ISO).
        </p>
      </Section>
      ...
      <Section id="registry-scope" icon={ListTree} title="What's inside: families and curves">
        <p>
          ...
          standardization references and naming-pattern templates for its parameterized variants
          (e.g. key length, hash choice).
        </p>
        <p>
          <Badge kind="std" />
          <strong>
      ...
      >
        <p>
          <Badge kind="std" />
          The registry ships as its own <strong>versioned JSON + JSON Schema</strong>, decoupled
          from the CycloneDX specification release cycle — it can add a new algorithm family without
          waiting for the next CycloneDX version.
        </p>
        <p>
          <Badge kind="view" />
          It is explicitly designed for use <em>outside</em> CBOM or CycloneDX entirely: an
          SPDX-based scanner, a homegrown inventory format, or a compliance dashboard can all adopt
          the same canonical names — CBOM is simply CycloneDX&apos;s own consumer of it.
        </p>
      </Section>
      ...
      <Section id="registry-practice" icon={Wrench} title="Using it in practice">
        <p>
          ...
          network capture — hands you a mechanism name in its own notation. The registry is the
          lookup table that resolves each one to a single canonical family or curve before it goes
          into your inventory, which is what makes a policy-as-code check (quantum-safe /
          quantum-vulnerable / unknown) match reliably across sources.
        </p>

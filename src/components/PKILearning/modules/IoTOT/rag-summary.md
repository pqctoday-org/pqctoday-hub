...
- **Algorithm selection for constrained devices**: ML-KEM-512 fits in ~3 KB stack RAM; FrodoKEM-640 needs ~60 KB stack RAM (RFC 7228 Class 2 is ~50 KiB, so infeasible across Class 0-2; the ~180 KB figure often quoted is FrodoKEM-1344's); FN-DSA-512 (FIPS 206 draft) produces the most compact PQC signature (~666 avg, 690 max bytes); LMS has smallest verifier (0.5 KB RAM, 56-byte public key) but requires stateful key management

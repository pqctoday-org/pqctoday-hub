  ...
  return (
    <div className="space-y-3">
      ...
      <svg viewBox="0 0 600 240" className="w-full h-56">
        ...
        />
        <text x={50} y={124} textAnchor="middle" className="fill-foreground text-[10px] font-bold">
          sk
        </text>
          ...
          return (
            ...
            >
              ...
              <rect
                ...
                strokeWidth={1}
              />
              <text x={180} y={y + 18} textAnchor="middle" className="fill-foreground text-[10px]">
                P{i + 1} simulate
              </text>
              {/* commitment box */}
              <rect
        ...
        >
          ...
          <text x={490} y={155} textAnchor="middle" className="fill-success text-[14px] font-bold">
            ✓
          </text>
        </motion.g>
...
const MultivariateVisual: React.FC = () => (
  <div className="space-y-3">
    <div className="text-sm font-semibold text-status-warning mb-1">
      Public map P : F<sup>n</sup> → F<sup>m</sup> looks random — secret is the hidden oil subspace
    </div>
    <svg viewBox="0 0 600 240" className="w-full h-56">
      <defs>
      ...
      >
        <circle
          ...
          strokeWidth={1}
        />
        <text x={140} y={172} textAnchor="middle" className="fill-foreground text-[9px] font-bold">
          v
        </text>
      ...
      >
        <rect
          ...
          strokeWidth={1}
        />
        <text x={467} y={63} textAnchor="middle" className="fill-foreground text-[10px] font-bold">
          Sign
        </text>
        ...
        <rect
          ...
          strokeWidth={2}
        />
        <text x={520} y={170} textAnchor="middle" className="fill-foreground text-[10px] font-bold">
          σ — 96–838 B
        </text>
      </motion.g>
    ...
    <p className="text-xs text-muted-foreground italic leading-relaxed">
      The verifier sees only the public map P — designed to look random. The signer knows where the
      oil subspace sits inside the variable space; substituting random vinegar collapses the
      quadratic system to a linear one in the oil coordinates, which Gaussian elimination solves in
      milliseconds. Wedge attacks broke that secrecy in some characteristic-2 instances, driving the
      move to odd-characteristic fields (QR-UOV survived unscathed).
    </p>
  </div>
)
...
const IsogenyVisual: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground italic leading-relaxed">
        The signer knows a secret isogeny <em>φ</em> from E₀ to E_pub — equivalently, a path through
        the supersingular graph. The signature is a compact algebraic representation of that
        morphism (an ideal in a quaternion order, not a literal node list), so it stays at 148 B
        even though the underlying graph has astronomic size. SQIsign avoids SIKE's pitfall by not
        revealing auxiliary torsion points to the verifier.
      </p>
    </div>
  )
}
  ...
  return (
    <div className="space-y-3">
      ...
      <svg viewBox="0 0 600 240" className="w-full h-56">
        ...
        />
        <text x={170} y={124} textAnchor="middle" className="fill-foreground text-[9px] font-bold">
          t
        </text>
      ...
      <p className="text-xs text-muted-foreground italic leading-relaxed">
        Both schemes solve the same problem — produce a short vector in a coset defined by a hash of
        the message. Falcon (FN-DSA) samples from a continuous Gaussian distribution and rounds to
        the lattice, which needs floating-point arithmetic that's notoriously hard to make

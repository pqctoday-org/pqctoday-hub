...
/**
 * Render-parity golden test for the PQCCandidates ModuleShell conversion.
 * Captured against the PRE-conversion module; must stay green after
 * PQCCandidates adopts <ModuleShell>, proving the conversion is
...
describe('PQCCandidates render parity', () => {
  it('renders the gradient header, the in-page description, and all six tabs', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'PQC Candidates & Standardisation Lifecycle' })
    ).toBeInTheDocument()
    // in-page description differs from the catalog description (the override slot)
    expect(screen.getByText(/How NIST evaluates new post-quantum mechanisms/)).toBeInTheDocument()

...
function computeRecommendations(inputs: CloudMatrixInputs): string[] {
  ...
  out.push(
    'PQC availability here reflects a 2025 provider-roadmap snapshot — verify current GA status with each provider before committing; this market moves quarterly. Refresh this matrix every 6 months, since FIPS 140-3 re-validation status also shifts the customer/provider boundary in practice.'
  )
...
export function renderCloudMatrixMarkdown(
  ...
  md += `---\n\n`
  md += `> **Data vintage:** PQC availability per provider reflects a 2025 provider-roadmap snapshot — verify current GA status with each provider.\n\n`

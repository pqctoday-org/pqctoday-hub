...
const LANGUAGE_CHECKLISTS: Record<string, string[]> = {
  ...
  C: [
    ...
    'OpenSSL 3.5+ LTS ships native FIPS 203 / 204 (ML-KEM, ML-DSA) support today via the FIPS provider (SLH-DSA / FIPS 205 still pending); for older OpenSSL 3.x, load the oqs-provider at run-time: `OSSL_PROVIDER_load(NULL, "oqsprovider")`',
...
const CURRENT_API_NOTES: Record<string, string> = {
  ...
  'dotnet-syscrypto':
    '.NET: ML-KEM / ML-DSA are native types from .NET 10 - prefer those over third-party providers; on .NET 8/9 use BouncyCastle.NET PQC.',
  'kms-backed':
...
// the Section 4 introduction (opening and closing sentences of that intro,
// separated by several other sentences), joined with an ellipsis rather than
// presented as one contiguous quotation. Note: this text precedes the 4.1
...
export function renderCryptoApiMarkdown(
  ...
  md += `*https://doi.org/10.6028/NIST.CSWP.39-upd1*\n\n`
  ...
  md += `---\n\n`
  md += `> "${CSWP39_41_QUOTE}"\n`
  md += `> -- NIST CSWP 39 Section 4 (introduction, preceding the 4.1 subsection)\n\n`
  md += `*Generated by PQC Today Hub. Standards citations: NIST CSWP 39 Section 4.1, Fig.2. https://doi.org/10.6028/NIST.CSWP.39-upd1*\n`

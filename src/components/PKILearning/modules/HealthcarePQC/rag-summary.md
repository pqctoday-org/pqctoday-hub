...
- **Hospital information systems**: EHR/EMR systems (HL7 FHIR R4/R5 API security — OAuth 2.0 + SMART on FHIR, token signing must migrate to ML-DSA), DICOM medical imaging (no DICOM supplement adding PQC digital signature support exists yet — DICOM Supplement 217 is titled "Neurophysiology Waveforms" and is unrelated; radiology report signatures are defined in DICOM PS3.15's Digital Signatures profile, which supports pluggable algorithm registration via OID, so ML-DSA would need a new signature algorithm identifier registered under that existing framework once proposed and ratified), telehealth TLS sessions (HIPAA requires encryption — TLS 1.3 + ML-KEM hybrid for video conferencing APIs), pharmacy dispensing systems (barcode/NFC verification, label authentication with ML-DSA-44)
...
- DICOM PS3.15 — Digital Signatures profile (pluggable algorithm registration; no PQC signature supplement exists yet)

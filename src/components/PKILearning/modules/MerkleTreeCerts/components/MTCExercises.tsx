...
export const MTCExercises: React.FC<MTCExercisesProps> = ({
  ...
  const scenarios: Scenario[] = [
    ...
    },
    {
      ...
      observe:
        'An inclusion proof depends only on how large its subtree is, never on the signature algorithm: ~96 bytes for this 8-leaf demo, 384 bytes for a standalone certificate reaching the latest checkpoint (~2,500 entries), 736 bytes for a landmark subtree of ~4.4M — the two figures the spec projects in §6.4. Because the proof is a fixed cost and the signature it replaces is not, MTC savings scale with signature size: ECDSA (64 B sig) saves 384 bytes standalone, ~31%; ML-DSA-44 (2,420 B sig) saves 7,590 bytes, ~63% standalone and ~92% landmark; SLH-DSA-128s (7,856 B sig) saves 15,902 bytes, ~65%. Watch what landmark mode does to ECDSA: at 936 bytes it is LARGER than the 841-byte standalone certificate, because a 736 B landmark proof costs more than every signature and key in a classical chain put together. MTCs are built for PQC migration — for compact classical algorithms the absolute saving is a few hundred bytes, and the signatureless mode is a net loss.',
      workshopStep: 3,
    },
  ]

...
This module addresses the certificate bloat problem created by post-quantum digital signatures and teaches how Merkle Tree Certificates (MTCs) solve it. Beyond size, MTCs serve a second critical function identified by Bas Westerbaan (Cloudflare Research): because MTC issuance is logged in a public, auditable transparency service, browsers and CAs can detect when a server that previously held an MTC-backed certificate suddenly falls back to a classical X.509 certificate — flagging potential quantum downgrade rollback attacks. PQC signatures are dramatically larger than classical ones: an ECDSA P-256 signature is 64 bytes while ML-DSA-44 is 2,420 bytes, a 37x increase. A typical PQC TLS certificate chain adds 18-36 KB of overhead, breaking constrained clients and degrading connection setup times. MTCs replace individual per-certificate signatures with a batch-signing approach using Merkle trees, achieving 31–65% standalone size reduction depending on algorithm (63–65% for ML-DSA/SLH-DSA chains; ~31% for ECDSA P-256, where the saving is only ~384 bytes in absolute terms). Note that landmark mode, which removes signatures entirely, is a net LOSS for ECDSA: its 736 B landmark proof costs more than the whole classical chain it replaces.
...
- **Inclusion proofs**: Each certificate holder receives a compact chain of sibling hashes from leaf to root. Size depends only on subtree size, and draft-ietf-plants-merkle-tree-certs-04 §6.4 projects two: 384 bytes for a standalone certificate's ~2,500-entry checkpoint subtree (12 hashes x 32 bytes) and 736 bytes for a landmark subtree of ~4.4 million (23 hashes x 32 bytes)
...
- **MTC as downgrade detection layer** (Westerbaan/Cloudflare): Because every MTC issuance is recorded in the public transparency log, any future absence of an MTC-backed cert for a previously enrolled domain is visible — monitors can flag a sudden reversion to classical X.509 as a potential rollback attack; this is distinct from the size-reduction motivation and makes MTC adoption a security property, not just a performance optimization
- **Chrome Quantum-Resistant Root Store (CQRS) coexistence**: Chrome's CQRS enables the browser to validate PQC certificates via a parallel root store that runs alongside (not replacing) the existing Web PKI root store; servers can present an MTC-backed PQC cert to CQRS-capable browsers while falling back to classical X.509 for legacy clients — maximum compatibility with an auditable quantum-safe path

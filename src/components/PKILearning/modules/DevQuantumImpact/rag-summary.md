...
- **Code Signing Pipeline Changes** — cosign (Sigstore) and Notation (CNCF) are exploring PQC signing (no committed versions or dates yet); ML-DSA for OpenPGP is standardized in RFC 9980 (RFC 9580 itself contains no ML-DSA); Docker Content Trust (Notary v1) has no PQC roadmap and should be migrated immediately

  ...
  <div className="space-y-5">
    ...
    <Section id="sbom-why" icon={Package} title="Why an SBOM, and what it isn't">
      ...
      <p>
        ...
        &apos;s job, one layer up. An SBOM lists software; a CBOM extends that list with algorithm,
        key size, protocol, and quantum-vulnerability fields the SBOM has no place for. Keep the two
        separate: this module is the software inventory, not the crypto one.
      </p>
    </Section>
    ...
    <Section id="sbom-formats" icon={FileJson} title="SPDX vs CycloneDX — general BOM formats">
      <p>
        <Badge kind="std" />
        <strong>SPDX</strong> (Linux Foundation; standardized as <strong>ISO/IEC 5962:2021</strong>)
        grew out of license-compliance tooling and has the deeper license/provenance model.{' '}
        <strong>CycloneDX</strong> (OWASP; standardized as <strong>ECMA-424</strong>) grew out of
        application-security tooling and is the more extensible object model — SaaSBOM, HBOM, ML-BOM
        and VDR/VEX are all sibling BOM types built on the same schema.
      </p>
      <p>
        <Badge kind="view" />
        Neither format has a cryptography object model built in for general use. CycloneDX does have
        a crypto-specific extension (assetType: cryptographic-asset) — that extension, and the
        SPDX-vs-CycloneDX trade-off <em>for a crypto inventory specifically</em>, is covered in the
        CBOM module, not repeated here. Likewise, CycloneDX&apos;s own cross-tool algorithm/curve
        naming registry is covered in the{' '}
    ...
    <Section id="sbom-vex" icon={Bug} title="VEX closes the vulnerability-triage gap">
      <p>
        <Badge kind="std" />
        An SBOM lists what is present; it says nothing about whether a listed component&apos;s known
        vulnerabilities are actually reachable in this product.{' '}
      ...
      <p>
        ...
        fires an alert for every product that merely lists it in an SBOM, regardless of whether the
        vulnerable code path is ever called. VEX turns that into a triage decision instead of a fire
        drill.
      </p>
    </Section>

    <Section id="sbom-regulation" icon={Scale} title="What's actually mandated: EO 14028 & EU CRA">
      <p>
        <Badge kind="std" />
      ...
      <p>
        ...
        reason a PQC migration program cares is downstream: an SBOM built to satisfy EO 14028 or the
        CRA is exactly the existing data source a crypto-discovery effort should integrate rather
        than duplicate.
      </p>
    </Section>
    ...
    <Section id="sbom-to-cbom" icon={ArrowRightLeft} title="Bridge: from SBOM to CBOM">
      <p>
        <Badge kind="view" />
        An SBOM is <strong>Phase 1 discovery input</strong>, not a Phase 2 output: it is one of the
        existing data sources a crypto-discovery effort should cross-reference rather than rebuild
        from scratch. Skipping that cross-reference — building a CBOM without linking it back to
        SBOM dependency chains — is a named common failure in PQC migration programs.
      </p>
      <p>
        <Badge kind="view" />

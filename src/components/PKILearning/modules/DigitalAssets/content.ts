...
export const content: ModuleContent = {
  ...
  narratives: {
    ...
    relatedStandards:
      "SEC 2 (secp256k1 curve specification), RFC 8032 (Ed25519/EdDSA), BIP-32 (HD wallet derivation), BIP-39 (mnemonic seed phrases), BIP-44 (multi-account hierarchy), SLIP-0010 (Ed25519 HD derivation), BIP-141 (Segregated Witness — P2WPKH/P2WSH, bech32 addresses), BIP-341 (Taproot — P2TR spending rules, Schnorr signatures), BIP-350 (bech32m encoding for Taproot/SegWit v1+ addresses). EIP-55 (Ethereum address checksumming), EIP-155 (replay protection with chain ID). FIPS 204 (ML-DSA), FIPS 203 (ML-KEM), FIPS 206 (FN-DSA — cited here via the Falcon v1.2 specification it standardises, pending NIST's final publication; the BIP-39/BIP-44 citations are deprecated pending re-capture of the source page, not because either standard changed) as potential PQC replacements for blockchain signature and key exchange schemes",
  },
}

...
export const content: ModuleContent = {
  ...
  narratives: {
    ...
    keyConcepts:
      "AES-256 is quantum-safe: Grover's algorithm reduces effective bits from 256 to 128, but AES-256 still meets NIST PQC Category 5 — the highest (AES-128 = Cat 1, AES-192 = Cat 3, AES-256 = Cat 5). AES-256 stays usable but offers no margin above that floor — AES-192 and AES-128 fall below it after Grover. No data file re-encryption needed. DEK wrapping is the PQC target: RSA-OAEP and ECDH used to wrap DEKs must be replaced with ML-KEM-1024. ML-KEM-1024 key sizes: Public key: 1,568 bytes (vs 256 bytes RSA-2048). Ciphertext: 1,568 bytes. ~6× metadata overhead per DEK — negligible for multi-TB databases.",
  },
}

  ...
  SUCI_STEPS_C: [
    {
      ...
      description:
        'For Profile C (Post-Quantum), the home network provisions ML-KEM key material. In Hybrid mode, two keypairs are generated: an ML-KEM-768 keypair (FIPS 203, lattice-based, quantum-resistant) for the KEM component, and an X25519 keypair for the classical ECDH component. The combined shared secret Z = SHA256(Z_ecdh ‖ Z_kem) provides security against both classical and quantum adversaries. In Pure PQC mode, only the ML-KEM-768 keypair is generated. Both private keys are held in the HSM for SIDF deconcealment.',
...
// Step C: Z = SHA256(Z_ecdh || Z_kem) — this project's hybrid combiner
const Z = hsm_digest(M, hSession, concat(zEcdhBytes, zKemBytes), CKM_SHA256)`,
    ...
    },
    {
      ...
      description:
        'The shared secret is passed through ANSI X9.63 KDF using SHA3-256. SHA3-256 replaces SHA-256 to ensure post-quantum security of the KDF itself. Two iterations produce K_enc (256-bit AES-256, full block1) and K_mac (256-bit HMAC-SHA3-256, full block2). SharedInfo = raw X25519 ephemeral public key (32 bytes) for hybrid mode; empty for pure PQC. This KDF choice was made here: no 3GPP document specifies one for a post-quantum SUCI scheme, and the research proposal this profile is named after leaves the KDF generic. There are no test vectors to check it against.',
      code: `# ANSI X9.63 KDF with SHA3-256, 2 iterations (this project's choice)
...
mac_key = block2  # 256-bit HMAC-SHA3-256 key (full block2)`,
      ...
      explanationTable: [
        {
          label: 'Standard',
          value: 'ANSI X9.63 KDF + FIPS 203 — no 3GPP profile',
          description:
            'TS 33.501 Annex C defines Profiles A and B only. "Profile C" is a research proposal (Khan, Purification & Chang, Information 2025, 16, 617); this KDF and the hybrid combiner were added here, not taken from a spec. No test vectors exist for any of it. Execute the step to see live computed values.',
        },
        {
          label: 'Z source',
        ...
        },
        {
          ...
          description:
            'Pure PQC mode omits SharedInfo because there is no ECDH ephemeral key. Nothing specifies this either way — it follows from there being no ephemeral EC key to bind in.',
        },
        {
          label: 'K_enc',
...
// Profile C here uses AES-256-CTR with zero IV (this project's choice; TS 33.501
// Annex C uses AES-128-CTR for Profiles A and B)
const ciphertext = hsm_aesEncrypt(
...
const macTagFull = hsm_hmac(
  hsmd, sessionHandle, hKmac, ciphertext, 'SHA3-256'
    ...
    },
    {
      ...
      title: '11. Network SIDF: Decrypt SUCI (Decryption Point)',
      description:
        'For Profile C, the SIDF re-derives the same combined shared secret and verifies integrity before decrypting. Authenticate-then-decrypt follows the ECIES pattern TS 33.501 Annex C uses for Profiles A and B: the MAC MUST pass before decryption begins.',
      code: `// SoftHSMv3 WASM: Profile C SIDF — Hybrid Deconcealment

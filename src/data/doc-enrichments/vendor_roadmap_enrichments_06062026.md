...
- **Extraction Timestamp**: 2026-06-06T20:28:53
...
- **Products / Services Covered**: SymCrypt; Windows; Linux; Microsoft Azure; Microsoft 365; Microsoft Entra; Azure Key Vault
- **Compliance Frameworks**: NIST; CNSA 2.0; CNSSP-15; ISO; IETF; OMB; CISA; NSA
- **Hybrid Mode Support**: Yes, TLS hybrid key exchange enabled in SymCrypt-OpenSSL 1.9.0; strategy includes hybrid approach combining classical and quantum-resistant algorithms as an interim step.
- **Current GA Status**: Preview
- **Customer Action Required**: start developing their strategy now; proactively prepare their software and services for PQC support
...
- **Coverage Verification**: CONSISTENT, the document explicitly confirms the SymCrypt foundation, ML-KEM/ML-DSA via CNG, TLS hybrid support, preview status, three-phase strategy, and 2029/2033 timelines.
- **Extraction Quality**: HIGH
- **Source Document**: VND-027_Microsoft_Corporation.html (313.6 KB)
- **Extraction Timestamp**: 2026-06-06T20:49:46
...
- **Coverage Verification**: CONSISTENT. The document confirms QID 38994 usage and coverage across VM, Certificate View, WAS, EASM, and VM_AUTH scans, matching the notes.
- **Extraction Quality**: MEDIUM
- **Source Document**: VND-164_Qualys_Inc..html (56.4 KB)
- **Extraction Timestamp**: 2026-06-06T20:56:21
...
- **Products / Services Covered**: Portfolio-wide commitment (no individual products named)
- **Compliance Frameworks**: NIST
- **Hybrid Mode Support**: Yes, mentions "hybrid ECC+ML-KEM key-exchange strategy" in coverage notes context, but document text only mentions "lattice-based cryptography such as ML-KEM... as potential replacements". The document text itself does not explicitly detail the hybrid implementation, only the algorithm. However, the prompt asks to extract from document text. The text says "lattice-based cryptography such as ML-KEM... are being developed as potential replacements". It does not explicitly state "hybrid mode" in the provided text snippet, though the coverage notes do. Strictly from text: No explicit mention of "hybrid" mode implementation details, only that ML-KEM is a replacement. Wait, the coverage notes are external. The text says "lattice-based cryptography such as ML-KEM... are being developed as potential replacements". It does not say "hybrid". I will mark No based on strict text extraction, or Partial if I consider the general context. Let's look closer. "Zscaler’s phased approach to post-quantum key exchange" is listed in future entries. The current text does not describe the hybrid mode. So: No.
- **Current GA Status**: Planned
...
- **Coverage Verification**: PARTIAL, the document confirms the blog series and date but does not contain the specific technical details (hybrid ECC+ML-KEM, inline inspection, IPsec PQC keys, SI partners) mentioned in the coverage notes, which appear to be from other parts of the series or separate announcements.
- **Extraction Quality**: MEDIUM
...
- **Roadmap Scope**: Portfolio-wide strategy
- **PQC Algorithms Announced**: ML-KEM; Classic McEliece
...
- **Current GA Status**: Planned
- **Customer Action Required**: Begin phased PQ rollout and policy updates immediately; Inventory current encryption usage; Identify long-lived secrets and retention policies
...
- **Coverage Verification**: PARTIAL; The document confirms the dual-layer strategy, algorithms, and 12-month roadmap phases, but does not mention an "agile post-quantum crypto layer" or "FIPS 140-3 Level 3" certification.
- **Extraction Quality**: HIGH

...
- **CSV Coverage Notes**: AWS Secrets Manager Agent (v2.0.0+), Lambda Extension (v19+), Secrets and Configuration Provider (v2.0.0+), and SDKs (Rust, Go, Node.js, Kotlin, Python, Java v2) now support hybrid post-quantum TLS combining classical key exchange with ML-KEM. GA; no code/config changes required for clients already on the latest versions. (Apr 2026)
...
- **Customer Action Required**: Upgrade to the latest client versions; check AWS CloudTrail logs for "X25519MLKEM768"
- **Key Commitments & Quotes**: "AWS Secrets Manager clients now support hybrid post-quantum TLS to protect secrets from quantum risks"; "No code changes, configuration updates, or migration effort are required for use cases that have already upgraded to the latest client versions"; "protect against both traditional cryptographic attacks and future quantum computing risks"
- **Coverage Verification**: CONSISTENT; The document explicitly confirms support for the listed agents, extensions, and SDKs with hybrid PQC TLS using ML-KEM, noting GA status and no code changes for latest versions.

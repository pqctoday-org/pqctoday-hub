...
enrichment_method: mlx_lm.server-mlx-community/Qwen3.8-27B-8bit
...
## VND-057:blog_cloudflare_com_post_quantum_roadmap — Cloudflare Inc.
...
- **Target Migration Dates**: Full post-quantum security including authentication by 2029
- **Products / Services Covered**: Cloudflare One
- **Compliance Frameworks**: None detected
- **Hybrid Mode Support**: No
- **Current GA Status**: GA (Post-quantum encryption for all websites and APIs enabled in 2022)
- **Customer Action Required**: None detected (Document states: "you do not need to take any mitigating action")
- **Key Commitments & Quotes**: "We now target 2029 to be fully post-quantum (PQ) secure including, crucially, post-quantum authentication."; "We are targeting full post-quantum security including authentication for our entire product suite by 2029."; "every post-quantum upgrade we build will continue to be available to all customers, on every plan, at no additional cost"
- **Coverage Verification**: CONSISTENT The document provides a general roadmap and timeline but does not specify detailed algorithmic or product-level coverage notes.
- **Extraction Quality**: MEDIUM
- **Source Document**: VND-057_Cloudflare_Inc.html (501.9 KB)
- **Extraction Timestamp**: 2026-09-13T15:31:49


## VND-057:blog_cloudflare_com_post_quantum_dnssec_ — Cloudflare Inc.

- **Vendor ID**: VND-057
- **Vendor Name**: Cloudflare Inc.
- **Roadmap Title**: 1.1.1.1 now supports post-quantum DNSSEC
- **Roadmap URL**: https://blog.cloudflare.com/post-quantum-dnssec-1111/
- **Publish Date**: 2026-09-10
- **Local File**: vendor-roadmaps/VND-057_Cloudflare_Inc_blog_cloudflare_com_post_quantum_dnssec_.html
- **CSV Coverage Notes**: Cloudflare blog (2026-09-10): 1.1.1.1 now validates DNSSEC signatures made with ML-DSA-44 (IANA DNSSEC algorithm 18, draft-westerbaan-dnssec-mldsa) by default, with a live ML-DSA-44-signed test zone (dnstest.dev). Resolver-side only: applies a local policy requiring at least one valid post-quantum path when the parent's DS RRset signals a PQ algorithm (downgrade protection). Next steps stated: ML-DSA-44 signing in Cloudflare Authoritative DNS and DS-record support in Cloudflare Registrar. Company-wide target unchanged: full post-quantum security by 2029. Second, separate row from the general 2029-roadmap announcement.
- **PQC Algorithms Announced**: ML-DSA-44
- **Target Migration Dates**: full post-quantum security by 2029
- **Products / Services Covered**: 1.1.1.1; Cloudflare Authoritative DNS; Cloudflare Registrar
- **Compliance Frameworks**: NIST; IANA; RFC 6840; RFC 4035; RFC 9715
- **Hybrid Mode Support**: Yes, zones publish conventional and post-quantum keys/signatures together for compatibility, with resolvers applying local policy to require PQ paths when signaled.
- **Current GA Status**: GA
- **Customer Action Required**: None detected
- **Key Commitments & Quotes**: "Cloudflare plans to achieve full post-quantum security by 2029"; "Our next step is adding ML-DSA-44 signing support to Cloudflare Authoritative DNS"; "corresponding DS record support to Cloudflare Registrar"
- **Coverage Verification**: CONSISTENT, the document confirms ML-DSA-44 validation in 1.1.1.1, the dnstest.dev zone, the 2029 target, and next steps for Authoritative DNS and Registrar.
- **Extraction Quality**: HIGH
- **Source Document**: VND-057_Cloudflare_Inc_blog_cloudflare_com_post_quantum_dnssec_.html (880.5 KB)
- **Extraction Timestamp**: 2026-09-13T15:31:49
...
- **Key Commitments & Quotes**: "AWS is migrating to post-quantum cryptography (PQC)."; "we will begin deployment of this s2n version across all AWS public endpoints that offer HTTPS-based interfaces."; "AWS will adopt ML-DSA... enabling customers to generate and use PQC keys"
- **Coverage Verification**: MISMATCH — The document explicitly details a multi-workstream migration plan covering specific algorithms, services, and timelines, contradicting the claim that coverage is "Not specified".
- **Extraction Quality**: HIGH
- **Source Document**: VND-001_Amazon_Web_Services_Inc.html (1253.9 KB)
- **Extraction Timestamp**: 2026-09-11T10:57:14
...
- **Hybrid Mode Support**: Yes; embeds both conventional and quantum-safe certificates in a single container, allowing quantum-safe algorithms to be turned on as needed while using conventional algorithms until then.
- **Current GA Status**: GA
- **Customer Action Required**: None detected
...
- **Coverage Verification**: CONSISTENT; The document explicitly states "BULVERDE, Texas, February 20, 2020" as the announcement date for the KMES Series 3, confirming the 2020-02-20 date in the claim.
...
- **Roadmap URL**: https://cloud.google.com/blog/products/identity-security/pqc-in-plaintext-google-clouds-post-quantum-cryptography-roadmap
- **Publish Date**: Unknown
- **Local File**: vendor/VND-018.html
- **CSV Coverage Notes**: None
- **PQC Algorithms Announced**: ML-KEM; ML-DSA; SLH-DSA
- **Target Migration Dates**: Full PQC readiness by 2029; Domain 1 (SNDL) mitigation by end of 2027; Domain 2 (Integrity) and Domain 3 (Foundations) by end of 2028
- **Products / Services Covered**: Google Cloud API endpoints; Application and proxy load balancers; Cloud KMS; Cloud VPN; Cloud Interconnect; GCE OS Login; Cloud SDK; gCloud CLI; GKE service mesh; client libraries; Cloud Storage SDK; Storage Transfer Service; BigQuery CLI; Data Transfer Service; Binary Authorization; Access Approval (AXA); Assured OSS; Certificate Authority Service (CAS); Google Trust Service; Cloud IAM; Confidential Compute; Cloud Hardware Security Module (HSM); External Key Management; Tink; BoringSSL; ALTS
- **Compliance Frameworks**: NIST FIPS 203; CNSA 2.0; NIST IR 8547; FIPS 140-3 L3
- **Hybrid Mode Support**: Yes, ML-KEM implemented in hybrid mode for API endpoints; X25519MLKEM768 hybrid key exchange for load balancers
- **Current GA Status**: GA (Cloud KMS PQC algorithms generally available; API endpoints and load balancers support active)
- **Customer Action Required**: Inventory cryptographic resources; Update development/SRE software to support PQC algorithms; Validate existing application behaviors using quantum-safe APIs and load balancers
- **Key Commitments & Quotes**: "We plan to achieve full PQC readiness by 2029"; "NIST standardized PQC algorithms (ML-KEM, ML-DSA, SLH-DSA) for your encryption and signing keys are now generally available"; "Google Cloud API endpoints now offer quantum-safe key exchange"
- **Coverage Verification**: MISMATCH — The document explicitly details a comprehensive roadmap with specific dates (2027-2029), algorithms, and services, contradicting the claim that coverage is "Not specified".
- **Extraction Quality**: HIGH
- **Source Document**: VND-018.html (338.8 KB)
- **Extraction Timestamp**: 2026-09-11T10:57:14

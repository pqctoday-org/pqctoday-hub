...
export interface PlaygroundTool {
  ...
  /**
   * Optional per-dimension footnote. Surfaced next to the matrix cell for
   * dimensions whose testability is non-trivial — e.g. a 'partial' that comes
   * from an educational construct rather than a standardized profile.
   */
  pureKemNote?: string
...
export const PROTOCOL_MATRIX: ProtocolMatrixRow[] = [
  ...
  },
  {
    ...
    dimensions: {
      ...
      hybridKem: {
        value: 'experimental',
        note: 'v1.85 Labeled KEM abstraction can mix algorithms; not standardized as "hybrid". The PQC TPM Workshop exposes an educational hybrid Labeled-KEM (ML-KEM + X25519/P-256, HKDF-SHA256 combiner) for teaching purposes only.',
      },
      pureSig: {
        value: 'rfc',
